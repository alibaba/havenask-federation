 ...
 * {@link HavenaskIntegTestCase} then this information is the node name.
 */
...
public class TestThreadInfoPatternConverter extends LogEventPatternConverter {
    ...
    private static final Pattern HAVENASK_THREAD_NAME_PATTERN =
            Pattern.compile("havenask\\[(.+)\\]\\[.+\\].+");
    ...
    static String threadInfo(String threadName) {
        Matcher m = HAVENASK_THREAD_NAME_PATTERN.matcher(threadName);
        if (m.matches()) {
