 ...
 */
public class HavenaskIndexInputTestCase extends HavenaskTestCase {

    private static HavenaskThreadPoolExecutor executor;

    @BeforeClass
    public static void createExecutor() {
        final String name = "TEST-" + getTestClass().getSimpleName() + "#randomReadAndSlice";
        executor = HavenaskExecutors.newFixed(
            name, 10, 0, HavenaskExecutors.daemonThreadFactory(name), new ThreadContext(Settings.EMPTY));
    }
