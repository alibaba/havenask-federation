            ...
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                if ("data".equals(currentFieldName)) {
                    if (parser.nextToken() != XContentParser.Token.VALUE_STRING) {
                        throw new HavenaskParseException("failed to parse snapshottable metadata, invalid data type");
                    }
                    data = parser.text();
                } else {
                    throw new HavenaskParseException("failed to parse snapshottable metadata, unknown field [{}]", currentFieldName);
                }
            } else {
                throw new HavenaskParseException("failed to parse snapshottable metadata");
            }
        }
        if (data == null) {
            throw new HavenaskParseException("failed to parse snapshottable metadata, data not found");
        }
