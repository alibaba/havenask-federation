...
public final class MockEngineSupport {
    ...
    public MockEngineSupport(EngineConfig config, Class<? extends FilterDirectoryReader> wrapper) {
        ...
        final long seed =  config.getIndexSettings().getValue(HavenaskIntegTestCase.INDEX_TEST_SEED_SETTING);
    ...
    private DirectoryReader wrapReader(DirectoryReader reader) {
        try {
            ...
            throw new HavenaskException("Can not wrap reader", e);
        }
        return reader;
    }
    ...
    private static final class InFlightSearchers implements Closeable {
        ...
        private final IdentityHashMap<Object, RuntimeException> havenaskers = new IdentityHashMap<>();

        @Override
        public synchronized void close() {
            if (havenaskers.isEmpty() == false) {
                AssertionError error = new AssertionError("Unreleased searchers found");
                for (RuntimeException ex : havenaskers.values()) {
        ...
        void add(Object key, String source) {
            ...
            synchronized (this) {
                havenaskers.put(key, ex);
            }
        }

        synchronized void remove(Object key) {
            havenaskers.remove(key);
        }
    }
