...
import org.havenask.secure_sm.SecureSM;
...
public class BootstrapForTesting {
    ...
    static {
        ...
        if (systemPropertyAsBoolean("tests.security.manager", true)) {
            try {
                ...
                // when testing server, the main havenask code is not yet in a jar, so we need to manually add it
                addClassCodebase(codebases,"havenask", "org.havenask.plugins.PluginsService");
                if (System.getProperty("tests.gradle") == null) {
                    // intellij and eclipse don't package our internal libs, so we need to set the codebases for them manually
                    addClassCodebase(codebases,"plugin-classloader", "org.havenask.plugins.ExtendedPluginsClassLoader");
                    addClassCodebase(codebases,"havenask-nio", "org.havenask.nio.ChannelFactory");
                    addClassCodebase(codebases, "havenask-secure-sm", "org.havenask.secure_sm.SecureSM");
                    addClassCodebase(codebases, "havenask-rest-client", "org.havenask.client.RestClient");
                }
                final Policy testFramework = Security.readPolicy(Bootstrap.class.getResource("test-framework.policy"), codebases);
                final Policy havenaskPolicy = new HavenaskPolicy(codebases, perms, getPluginPermissions(), true, new Permissions());
                Policy.setPolicy(new Policy() {
                    @Override
                    public boolean implies(ProtectionDomain domain, Permission permission) {
                        // implements union
                        return havenaskPolicy.implies(domain, permission) || testFramework.implies(domain, permission);
                    }
                });
                ...
                for (URL url : Collections.list(
                        BootstrapForTesting.class.getClassLoader().getResources(PluginInfo.HAVENASK_PLUGIN_PROPERTIES))) {
    ...
    static Map<String,Policy> getPluginPermissions() throws Exception {
        List<URL> pluginPolicies =
            Collections.list(BootstrapForTesting.class.getClassLoader().getResources(PluginInfo.HAVENASK_PLUGIN_POLICY));
