...
HAVENASK_ZONE=$(echo $BUILD_ZONE | cut -d '.' -f 1,2)
...
sed -i 's/${HAVENASK_ZONE}/'$HAVENASK_ZONE'/g' $LOCALSTATEDIR/krb5.conf
...
kadmin.local -q "addprinc -pw havenask admin/admin@$REALM_NAME"
