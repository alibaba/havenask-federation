...
Vagrant.configure(2) do |config|
  ...
  # /havenask because /vagrant is confusing when there is a project inside
  # the havenask project called vagrant....
  config.vm.synced_folder '.', '/vagrant', disabled: true
  config.vm.synced_folder '.', '/havenask'
...
def linux_common(config,
  ...
  config.vm.provision 'clean havenask installs in tmp', run: 'always', type: 'shell', inline: <<-SHELL
    rm -rf /tmp/havenask*
  SHELL
...
def sh_set_prompt(config, name)
  config.vm.provision 'set prompt', type: 'shell', inline: <<-SHELL
      cat \<\<PROMPT > /etc/profile.d/havenask_prompt.sh
export PS1='#{name}:\\w$ '
PROMPT
      grep 'source /etc/profile.d/havenask_prompt.sh' ~/.bashrc |
        cat \<\<SOURCE_PROMPT >> ~/.bashrc
# Replace the standard prompt with a consistent one
source /etc/profile.d/havenask_prompt.sh
SOURCE_PROMPT
      grep 'source /etc/profile.d/havenask_prompt.sh' ~vagrant/.bashrc |
...
def sh_install_deps(config,
  ...
  config.vm.provision 'install dependencies', type: 'shell', inline:  <<-SHELL
    ...
    cat \<\<SUDOERS_VARS > /etc/sudoers.d/havenask_vars
Defaults   env_keep += "JAVA_HOME"
Defaults   env_keep += "SYSTEM_JAVA_HOME"
SUDOERS_VARS
    chmod 0440 /etc/sudoers.d/havenask_vars
  SHELL
end
