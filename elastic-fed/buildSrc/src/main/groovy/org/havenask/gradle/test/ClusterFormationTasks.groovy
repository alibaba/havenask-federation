...
import org.havenask.gradle.plugin.PluginBuildPlugin
import org.havenask.gradle.plugin.PluginPropertiesExtension
...
class ClusterFormationTasks {
    ...
    static List<NodeInfo> setup(Project project, String prefix, Task runner, ClusterConfiguration config) {
        ...
        Configuration currentDistro = project.configurations.create("${prefix}_havenaskDistro")
        Configuration bwcDistro = project.configurations.create("${prefix}_havenaskBwcDistro")
        Configuration bwcPlugins = project.configurations.create("${prefix}_havenaskBwcPlugins")
        ...
        configureDistributionDependency(project, config.distribution, currentDistro, VersionProperties.getHavenask())
        ...
        for (int i = 0; i < config.numNodes; i++) {
            ...
            String havenaskVersion
            if (i < config.numBwcNodes) {
                havenaskVersion = config.bwcVersion.toString()
                if (project.bwcVersions.unreleased.contains(config.bwcVersion)) {
                    havenaskVersion += "-SNAPSHOT"
                }
                distro = bwcDistro
            } else {
                havenaskVersion = VersionProperties.getHavenask()
                distro = currentDistro
            }
            NodeInfo node = new NodeInfo(config, i, project, prefix, havenaskVersion, sharedDir)
            nodes.add(node)
    ...
    static void configureDistributionDependency(Project project, String distro, Configuration configuration, String havenaskVersion) {
        ...
        if (distro.equals("integ-test-zip")) {
            ...
            if (internalBuild) {
                ...
                project.dependencies.add(
                        configuration.name,
                        "org.havenask.distribution.integ-test-zip:havenask:${havenaskVersion}@zip"
                )
            }
            return
        }
        ...
        if (unreleasedInfo != null) {
        ...
        } else if (internalBuild && havenaskVersion.equals(VersionProperties.getHavenask())) {
            ...
            dependency = "dnm:${artifactName}:${havenaskVersion}${classifier}@${packaging}"
        }
        project.dependencies.add(configuration.name, dependency)
    }

    /** Adds a dependency on a different version of the given plugin, which will be retrieved using gradle's dependency resolution */
    static void configureBwcPluginDependency(Project project, Object plugin, Configuration configuration, Version havenaskVersion) {
        if (plugin instanceof Project) {
            Project pluginProject = (Project)plugin
            verifyProjectHasBuildPlugin(configuration.name, havenaskVersion, project, pluginProject)
            final String pluginName = findPluginName(pluginProject)
            project.dependencies.add(configuration.name, "org.havenask.plugin:${pluginName}:${havenaskVersion}@zip")
        } else {
    ...
    /**
     * Adds dependent tasks to start an havenask cluster before the given task is executed,
     ...
     *   <li>Extract a fresh copy of havenask</li>
     *   <li>Write an havenask.yml config file</li>
    ...
    static Task configureNode(Project project, String prefix, Task runner, Object dependsOn, NodeInfo node, ClusterConfiguration config,
        ...
        if (node.config.plugins.isEmpty() == false) {
            if (node.nodeVersion == Version.fromString(VersionProperties.getHavenask())) {
    ...
    /** Adds a task to extract the havenask distribution */
    static Task configureExtractTask(String name, Project project, Task setup, NodeInfo node,
        ...
        /* configuration.singleFile will be an external artifact if this is being run by a plugin not living in the
          havenask source tree. If this is a plugin built in the havenask source tree or this is a distro in
          the havenask source tree then this should be the version of havenask built by the source tree.
    ...
    /** Adds a task to write havenask.yml for the given node configuration */
    static Task configureWriteConfigTask(String name, Project project, Task setup, NodeInfo node, Closure<Map> configFilter) {
        ...
        writeConfig.doFirst {
            ...
            File configFile = new File(node.pathConf, 'havenask.yml')
    ...
    static Task configureCreateKeystoreTask(String name, Project project, Task setup, NodeInfo node) {
        if (node.config.keystoreSettings.isEmpty() && node.config.keystoreFiles.isEmpty()) {
             ...
             */
            final Object esKeystoreUtil = "${-> node.binPath().resolve('havenask-keystore').toString()}"
    ...
    static Task configureCopyBwcPluginsTask(String name, Project project, Task setup, NodeInfo node, String prefix) {
        Configuration bwcPlugins = project.configurations.getByName("${prefix}_havenaskBwcPlugins")
        for (Map.Entry<String, Object> plugin : node.config.plugins.entrySet()) {
    ...
    static Task configureInstallModuleTask(String name, Project project, Task setup, NodeInfo node, Project module) {
        ...
        if (module.plugins.hasPlugin(PluginBuildPlugin) == false) {
            throw new GradleException("Task ${name} cannot include module ${module.path} which is not an havenaskplugin")
        }
    ...
    static Task configureInstallPluginTask(String name, Project project, Task setup, NodeInfo node, String pluginName, String prefix) {
         ...
         * the short name requiring the path to already exist.
         */
        final Object esPluginUtil = "${-> node.binPath().resolve('havenask-plugin').toString()}"
    ...
    /** Adds a task to start an havenask node with the given configuration */
    static Task configureStartTask(String name, Project project, Task setup, NodeInfo node) {
        ...
        Closure antRunner = { AntBuilder ant ->
            ant.exec(executable: node.executable, spawn: node.config.daemonize, newenvironment: true,
                     dir: node.cwd, taskname: 'havenask') {
        ...
        // this closure is the actual code to run havenask
        Closure havenaskRunner = {
            // Due to how ant exec works with the spawn option, we lose all stdout/stderr from the
            // process executed. To work around this, when spawning, we wrap the havenask start
            // command inside another shell script, which simply internally redirects the output
            // of the real havenask script. This allows ant to keep the streams open with the
            // dummy process, but us to have the output available if there is an error in the
            // havenask start script
            if (node.config.daemonize) {
        ...
        start.doFirst {
            ...
            List<String> havenaskJavaOpts = [node.env.get('HAVENASK_JAVA_OPTS', '')]
            String collectedSystemProperties = node.config.systemProperties.collect { key, value -> "-D${key}=${value}" }.join(" ")
            havenaskJavaOpts.add(collectedSystemProperties)
            havenaskJavaOpts.add(node.config.jvmArgs)
            if (Boolean.parseBoolean(System.getProperty('tests.asserts', 'true'))) {
                ...
                havenaskJavaOpts.add("-ea")
                havenaskJavaOpts.add("-esa")
            }
            // we must add debug options inside the closure so the config is read at execution time, as
            // gradle task options are not processed until the end of the configuration phase
            if (node.config.debug) {
                println 'Running havenask in debug mode, suspending until connected on port 8000'
                havenaskJavaOpts.add('-agentlib:jdwp=transport=dt_socket,server=y,suspend=y,address=8000')
            }
            node.env['HAVENASK_JAVA_OPTS'] = havenaskJavaOpts.join(" ")
    ...
    static Task configureWaitTask(String name, Project project, List<NodeInfo> nodes, List<Task> startTasks, int waitSeconds) {
        ...
        wait.doLast {
            ...
            if (ant.properties.containsKey("failed${name}".toString())) {
                waitFailed(project, nodes, logger, "Failed to start havenask: timed out after ${waitSeconds} seconds")
            }

            boolean anyNodeFailed = false
            for (NodeInfo node : nodes) {
                if (node.failedMarker.exists()) {
                    logger.error("Failed to start havenask: ${node.failedMarker.toString()} exists")
                    anyNodeFailed = true
                }
            }
            if (anyNodeFailed) {
                waitFailed(project, nodes, logger, 'Failed to start havenask')
            }
            ...
            if (missingFile) {
                waitFailed(project, nodes, logger, 'Havenask did not complete startup in time allotted')
            }
            ...
            for (NodeInfo node : nodes) {
                ...
                if (success == false) {
                    waitFailed(project, nodes, logger, 'Havenask cluster failed to pass wait condition')
                }
            }
        }
        return wait
    ...
    /** Adds a task to check if the process with the given pidfile is actually havenask */
    static Task configureCheckPreviousTask(String name, Project project, Object depends, NodeInfo node) {
        return project.tasks.create(name: name, type: Exec, dependsOn: depends) {
            ...
            doLast {
                ...
                if (out.contains("${ext.pid} org.havenask.bootstrap.Havenask") == false) {
                    logger.error('jps -l')
                    logger.error(out)
                    logger.error("pid file: ${node.pidFile}")
                    logger.error("pid: ${ext.pid}")
                    throw new GradleException("jps -l did not report any process with org.havenask.bootstrap.Havenask\n" +
    ...
    /** Adds a task to kill an havenask node with the given pidfile */
    ...
    static void verifyProjectHasBuildPlugin(String name, Version version, Project project, Project pluginProject) {
        if (pluginProject.plugins.hasPlugin(PluginBuildPlugin) == false) {
            throw new GradleException("Task [${name}] cannot add plugin [${pluginProject.path}] with version [${version}] to project's " +
                    "[${project.path}] dependencies: the plugin is not an havenaskplugin")
        }
    }

    /** Find the plugin name in the given project. */
    static String findPluginName(Project pluginProject) {
        PluginPropertiesExtension extension = pluginProject.extensions.findByName('havenaskplugin')
