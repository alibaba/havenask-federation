...
/** Configuration for an havenask cluster, used for integration tests. */
class ClusterConfiguration {
    ...
    /** Add a module to the cluster. The project must be an havenaskplugin and have a single zip default artifact. */
    @Input
    ...
    void extraConfigFile(String path, Object sourceFile) {
        if (path == 'havenask.yml') {
            throw new GradleException('Overwriting havenask.yml is not allowed, add additional settings using cluster { setting "foo", "bar" }')
        }
