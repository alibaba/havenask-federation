...
package org.havenask.gradle.vagrant;

import org.apache.commons.io.output.TeeOutputStream;
import org.havenask.gradle.LoggedExec;
import org.havenask.gradle.LoggingOutputStream;
...
public class VagrantMachine {
    ...
    // convert the given path from an havenask repo path to a VM path
    public static String convertLinuxPath(Project project, String path) {
        return "/havenask/" + project.getRootDir().toPath().relativize(Paths.get(path));
    }

    public static String convertWindowsPath(Project project, String path) {
        return "C:\\havenask\\" + project.getRootDir().toPath().relativize(Paths.get(path)).toString().replace('/', '\\');
    }
