...
public class ThirdPartyAuditPrecommitPlugin extends PrecommitPlugin {
    ...
    public static final String LIBS_HAVENASK_CORE_PROJECT_PATH = ":libs:havenask-core";
    ...
    public TaskProvider<? extends Task> createTask(Project project) {
        ...
        if (BuildParams.isInternal() && project.getPath().equals(":libs:havenask-core") == false) {
            // External plugins will depend on this already via transitive dependencies.
            // Internal projects are not all plugins, so make sure the check is available
            // we are not doing this for this project itself to avoid jar hell with itself
            project.getDependencies().add(JDK_JAR_HELL_CONFIG_NAME, project.project(LIBS_HAVENASK_CORE_PROJECT_PATH));
        }

        TaskProvider<ExportHavenaskBuildResourcesTask> resourcesTask = project.getTasks()
            .register("thirdPartyAuditResources", ExportHavenaskBuildResourcesTask.class);
