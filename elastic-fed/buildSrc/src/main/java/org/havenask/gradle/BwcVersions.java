...
/**
 * A container for havenask supported version information used in BWC testing.
 *
...
public class BwcVersions {
    ...
    public BwcVersions(List<String> versionLines) {
        this(versionLines, Version.fromString(VersionProperties.getHavenask()));
    }
    ...
    public BwcVersions(SortedSet<Version> allVersions, Version currentVersionProperty) {
        ...
        groupByMajor = allVersions.stream()
            ...
            // Adjust the major number since Havenask 1.x is released after predecessor version 7.x
            .filter(
    ...
    public List<Version> getIndexCompatible() {
        ...
        List<Version> result = Stream.concat(groupByMajor.get(prevMajor).stream(), groupByMajor.get(currentMajor).stream())
            ...
            .collect(Collectors.toList());
        if (currentMajor == 1) {
            // add 6.x compatible for Havenask 1.0.0
    ...
    public List<Version> getWireCompatible() {
        ...
        // if current is Havenask 1.0.0 add all of the 7.x line:
        if (currentMajor == 1) {
