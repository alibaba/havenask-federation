...
import org.havenask.search.aggregations.pipeline.SimpleValue;
...
import static org.havenask.search.aggregations.PipelineAggregatorBuilders.bucketScript;
...
public class MoreExpressionIT extends HavenaskIntegTestCase {
    ...
    public void testDateMethods() throws Exception {
        HavenaskAssertions.assertAcked(prepareCreate("test").addMapping("doc", "date0", "type=date", "date1", "type=date"));
        ensureGreen("test");
        indexRandom(true,
    ...
    public void testMultiValueMethods() throws Exception {
        HavenaskAssertions.assertAcked(prepareCreate("test").addMapping("doc",
                "double0", "type=double",
                "double1", "type=double",
                "double2", "type=double"));
        ensureGreen("test");
    ...
    public void testInvalidDateMethodCall() throws Exception {
        HavenaskAssertions.assertAcked(prepareCreate("test").addMapping("doc", "double", "type=double"));
        ensureGreen("test");
    ...
    public void testSparseField() throws Exception {
        HavenaskAssertions.assertAcked(prepareCreate("test").addMapping("doc", "x", "type=long", "y", "type=long"));
        ensureGreen("test");
        indexRandom(true,
        ...
        HavenaskAssertions.assertSearchResponse(rsp);
        SearchHits hits = rsp.getHits();
        assertEquals(2, rsp.getHits().getTotalHits().value);
        assertEquals(5.0, hits.getAt(0).field("foo").getValue(), 0.0D);
