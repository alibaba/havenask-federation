...
package org.havenask.ingest.useragent;
...
final class UserAgentParser {
    ...
    UserAgentParser(String name, InputStream regexStream, UserAgentCache cache) {
        ...
        try {
            ...
            throw new HavenaskParseException("error parsing regular expression file", e);
        }
    }
    ...
    private void init(InputStream regexStream) throws IOException {
        ...
        if (uaPatterns.isEmpty() && osPatterns.isEmpty() && devicePatterns.isEmpty()) {
            throw new HavenaskParseException("not a valid regular expression file");
        }
    }
    ...
    private List<Map<String, String>> readParserConfigurations(XContentParser yamlParser) throws IOException {
        ...
        if (token != XContentParser.Token.START_ARRAY) {
            throw new HavenaskParseException("malformed regular expression file, should continue with 'array' after 'object'");
        }

        token = yamlParser.nextToken();
        if (token != XContentParser.Token.START_OBJECT) {
            throw new HavenaskParseException("malformed regular expression file, expecting 'object'");
        }
        ...
        while (token == XContentParser.Token.START_OBJECT) {
            ...
            if (token != XContentParser.Token.FIELD_NAME) {
                throw new HavenaskParseException("malformed regular expression file, should continue with 'field_name' after 'array'");
            }
