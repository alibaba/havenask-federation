...
import org.havenask.action.DocWriteResponse.Result;
...
import org.havenask.action.search.ReduceSearchPhaseException;
...
import static org.havenask.action.bulk.BackoffPolicy.constantBackoff;
...
public class AsyncBulkByScrollActionTests extends HavenaskTestCase {
    ...
    public void testStartRetriesOnRejectionButFailsOnTooManyRejections() throws Exception {
        ...
        assertThat(ExceptionsHelper.stackTrace(e), containsString(HavenaskRejectedExecutionException.class.getSimpleName()));
        ...
        assertExactlyOnce(
            onFail -> {
                Consumer<Exception> validingOnFail = e -> {
                    assertNotNull(ExceptionsHelper.unwrap(e, HavenaskRejectedExecutionException.class));
                    onFail.run();
                };
                ClientScrollableHitSource hitSource = new ClientScrollableHitSource(logger, buildTestBackoffPolicy(),
                    threadPool,
    ...
    public void testThreadPoolRejectionsAbortRequest() throws Exception {
        ...
        setupClient(new TestThreadPool(getTestName()) {
            ...
            public ScheduledCancellable schedule(Runnable command, TimeValue delay, String name) {
                ...
                final HavenaskRejectedExecutionException exception = new HavenaskRejectedExecutionException("test");
        ...
        ExecutionException e = expectThrows(ExecutionException.class, () -> listener.get());
        assertThat(e.getCause(), instanceOf(HavenaskRejectedExecutionException.class));
    ...
    private class MyMockClient extends FilterClient {
                    ...
                    if (i == toReject) {
                        responses[i] = new BulkItemResponse(i, item.opType(),
                                new Failure(response.getIndex(), response.getType(), response.getId(),
                                    new HavenaskRejectedExecutionException()));
                    } else {
        ...
        private Exception wrappedRejectedException() {
            Exception e = new HavenaskRejectedExecutionException();
