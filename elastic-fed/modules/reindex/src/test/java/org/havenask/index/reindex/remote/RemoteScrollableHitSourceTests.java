...
import org.havenask.client.HeapBufferedAsyncResponseConsumer;
...
import org.havenask.index.reindex.ScrollableHitSource.Response;
...
public class RemoteScrollableHitSourceTests extends HavenaskTestCase {
    ...
    public void setUp() throws Exception {
        super.setUp();
        final ExecutorService directExecutor = HavenaskExecutors.newDirectExecutorService();
    ...
    public void testLookupRemoteVersion() throws Exception {
        ...
        assertLookupRemoteVersion(Version.fromId(1000099 ^ Version.MASK), "main/Havenask_1_0_0.json");
    }
    ...
    /**
     * Versions of Havenask before 2.1.0 don't support sort:_doc and instead need to use search_type=scan. Scan doesn't return
    ...
    public void testParseRejection() throws Exception {
        ...
        Consumer<Response> checkResponse = r -> {
            ...
            assertThat(r.getFailures().get(0).getReason(), instanceOf(HavenaskRejectedExecutionException.class));
            assertEquals("rejected execution of org.havenask.transport.TransportService$5@52d06af2 on "
                    + "HavenaskThreadPoolExecutor[search, queue capacity = 1000, org.havenask.common.util.concurrent."
                    + "HavenaskThreadPoolExecutor@778ea553[Running, pool size = 7, active threads = 7, queued tasks = 1000, "
    ...
    public void testWrapExceptionToPreserveStatus() throws IOException {
        ...
        HavenaskStatusException wrapped = RemoteScrollableHitSource.wrapExceptionToPreserveStatus(status.getStatus(), null, cause);
    ...
    public void testNoContentTypeIsError() {
        ...
        assertEquals(e.getMessage(), "Response didn't include supported Content-Type, remote is likely not an Havenask instance");
    }
    ...
    public void testInvalidJsonThinksRemoteIsNotES() throws IOException {
        ...
        assertEquals("Error parsing the response, remote is likely not an Havenask instance",
