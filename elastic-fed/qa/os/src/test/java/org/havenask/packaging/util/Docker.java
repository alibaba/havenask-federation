...
import static org.havenask.packaging.util.FileMatcher.p770;
import static org.havenask.packaging.util.FileMatcher.p775;
...
/**
 * Utilities for running packaging tests against the Havenask Docker images.
 */
public class Docker {
    ...
    /**
     * Runs an Havenask Docker container.
    ...
    /**
     * Runs an Havenask Docker container, with options for overriding the config directory
    ...
    public static Installation runContainer(
        ...
        waitForHavenaskToStart();

        return Installation.ofContainer(dockerShell, distribution);
    }

    /**
     * Similar to {@link #runContainer(Distribution, Map, Map)} in that it runs an Havenask Docker
        ...
        if (volumes != null) {
            volumes.forEach((localPath, containerPath) -> {
                ...
                if (Platforms.WINDOWS == false && System.getProperty("user.name").equals("root") && uid == null) {
                    // The tests are running as root, but the process in the Docker container runs as `havenask` (UID 1000),
    ...
    /**
     * Waits for the Havenask process to start executing in the container.
     * This is called every time a container is started.
     */
    public static void waitForHavenaskToStart() {
        boolean isHavenaskRunning = false;
        int attempt = 0;
        ...
        do {
            try {
                ...
                if (psOutput.contains("org.havenask.bootstrap.Havenask")) {
                    isHavenaskRunning = true;
                    break;
                }
            } catch (Exception e) {
        ...
        if (isHavenaskRunning == false) {
            final Shell.Result dockerLogs = getContainerLogs();
            fail(
                "Havenask container did not start successfully.\n\nps output:\n"
                    + psOutput
                    + "\n\nStdout:\n"
                    + dockerLogs.stdout
                    + "\n\nStderr:\n"
    ...
    /**
     * Waits for the Havenask container to exit.
     */
    private static void waitForHavenaskToExit() {
        boolean isHavenaskRunning = true;
        int attempt = 0;
        ...
        if (isHavenaskRunning) {
            final Shell.Result dockerLogs = getContainerLogs();
            fail("Havenask container did exit.\n\nStdout:\n" + dockerLogs.stdout + "\n\nStderr:\n" + dockerLogs.stderr);
        }
    }
    ...
    public static class DockerShell extends Shell {
        ...
        protected String[] getScriptCommand(String script) {
            ...
            return super.getScriptCommand("docker exec --user havenask:root --tty " + containerId + " " + script);
        }
    }
    ...
    public static void assertPermissionsAndOwnership(Path path, Set<PosixFilePermission> expectedPermissions) {
        ...
        assertThat("File owner of " + path + " is wrong", username, equalTo("havenask"));
    ...
    private static void verifyInstallation(Installation es) {
        dockerShell.run("id havenask");
        dockerShell.run("getent group havenask");

        final Shell.Result passwdResult = dockerShell.run("getent passwd havenask");
        final String homeDir = passwdResult.stdout.trim().split(":")[5];
        assertThat(homeDir, equalTo("/usr/share/havenask"));
        ...
        assertThat(dockerShell.run(es.bin("havenask-keystore") + " list").stdout, containsString("keystore.seed"));

        Stream.of(es.bin, es.lib).forEach(dir -> assertPermissionsAndOwnership(dir, p755));

        Stream.of("havenask", "havenask-cli", "havenask-env", "havenask-keystore", "havenask-shard", "havenask-plugin", "havenask-node")
    ...
    public static void waitForHavenask(Installation installation) throws Exception {
        withLogging(() -> ServerUtils.waitForHavenask(installation));
    }

    public static void waitForHavenask(String status, String index, Installation installation, String username, String password)
        throws Exception {
        withLogging(() -> ServerUtils.waitForHavenask(status, index, installation, username, password));
    }
    ...
    private static <E extends Exception> void withLogging(CheckedRunnable<E> r) throws Exception {
        try {
            ...
            logger.warn("Havenask container failed to start.\n\nStdout:\n" + logs.stdout + "\n\nStderr:\n" + logs.stderr);
            throw e;
        }
    }
