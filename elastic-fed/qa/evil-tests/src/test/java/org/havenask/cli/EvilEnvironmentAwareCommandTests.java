...
public class EvilEnvironmentAwareCommandTests extends HavenaskTestCase {

    @Rule
    public TestRule restoreSystemProperties = new TestRuleRestoreSystemProperties("havenask.path.conf");
    ...
    public void testEsPathConfNotSet() throws Exception {
        ...
        assertThat(e, hasToString(containsString("the system property [havenask.path.conf] must be set")));
    }

    @SuppressForbidden(reason =  "clears system property havenask.path.conf as part of test setup")
    private void clearEsPathConf() {
        System.clearProperty("havenask.path.conf");
    }
