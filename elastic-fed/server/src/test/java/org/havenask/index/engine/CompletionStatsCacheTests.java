...
public class CompletionStatsCacheTests extends HavenaskTestCase {

    public void testExceptionsAreNotCached() {
        final AtomicInteger openCount = new AtomicInteger();
        final CompletionStatsCache completionStatsCache = new CompletionStatsCache(() -> {
            throw new HavenaskException("simulated " + openCount.incrementAndGet());
        });

        assertThat(expectThrows(HavenaskException.class, completionStatsCache::get).getMessage(), equalTo("simulated 1"));
        assertThat(expectThrows(HavenaskException.class, completionStatsCache::get).getMessage(), equalTo("simulated 2"));
    }
