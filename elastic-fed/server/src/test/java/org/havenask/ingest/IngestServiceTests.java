...
public class IngestServiceTests extends HavenaskTestCase {
    ...
    public void testValidate() throws Exception {
        ...
        HavenaskParseException e =
            expectThrows(HavenaskParseException.class, () -> ingestService.validatePipeline(ingestInfos, putRequest));
        assertEquals("Processor type [remove] is not installed on node [" + node2 + "]", e.getMessage());
        assertEquals("remove", e.getMetadata("havenask.processor_type").get(0));
        assertEquals("tag2", e.getMetadata("havenask.processor_tag").get(0));
