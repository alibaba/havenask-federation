...
public class UpdateThreadPoolSettingsTests extends HavenaskThreadPoolTestCase {
    ...
    public void testWriteThreadPoolsMaxSize() throws InterruptedException {
        final int maxSize = 1 + HavenaskExecutors.allocatedProcessors(Settings.EMPTY);
    ...
    private static int getExpectedThreadPoolSize(Settings settings, String name, int size) {
        if (name.equals(ThreadPool.Names.WRITE) || name.equals(Names.SYSTEM_WRITE)) {
            return Math.min(size, HavenaskExecutors.allocatedProcessors(settings));
        } else {
            return size;
        }
    }
    ...
    public void testFixedExecutorType() throws InterruptedException {
        ...
        try {
            ...
            assertThat(threadPool.executor(threadPoolName), instanceOf(HavenaskThreadPoolExecutor.class));

            assertEquals(info(threadPool, threadPoolName).getThreadPoolType(), ThreadPool.ThreadPoolType.FIXED);
            assertThat(threadPool.executor(threadPoolName), instanceOf(HavenaskThreadPoolExecutor.class));
            assertThat(((HavenaskThreadPoolExecutor) threadPool.executor(threadPoolName)).getCorePoolSize(), equalTo(expectedSize));
            assertThat(((HavenaskThreadPoolExecutor) threadPool.executor(threadPoolName)).getMaximumPoolSize(), equalTo(expectedSize));
            ...
            assertThat(((HavenaskThreadPoolExecutor) threadPool.executor(threadPoolName)).getKeepAliveTime(TimeUnit.MINUTES),
                equalTo(0L));
        } finally {
            terminateThreadPoolIfNeeded(threadPool);
        }
        ...
        try {
            ...
            final ScalingExecutorBuilder scaling =
                new ScalingExecutorBuilder(
                    "my_pool1",
                    1,
                    HavenaskExecutors.allocatedProcessors(Settings.EMPTY),
