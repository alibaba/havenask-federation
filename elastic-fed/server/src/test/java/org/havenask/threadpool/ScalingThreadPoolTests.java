...
public class ScalingThreadPoolTests extends HavenaskThreadPoolTestCase {
    ...
    public void testScalingThreadPoolConfiguration() throws InterruptedException {
        ...
        runScalingThreadPoolTest(builder.build(), (clusterSettings, threadPool) -> {
            ...
            assertThat(executor, instanceOf(HavenaskThreadPoolExecutor.class));
            final HavenaskThreadPoolExecutor havenaskThreadPoolExecutor = (HavenaskThreadPoolExecutor)executor;
            ...
            assertThat(havenaskThreadPoolExecutor.getKeepAliveTime(TimeUnit.SECONDS), equalTo(keepAlive));

            assertNull(info.getQueueSize());
            assertThat(havenaskThreadPoolExecutor.getQueue().remainingCapacity(), equalTo(Integer.MAX_VALUE));

            assertThat(info.getMin(), equalTo(core));
            assertThat(havenaskThreadPoolExecutor.getCorePoolSize(), equalTo(core));
            assertThat(info.getMax(), equalTo(expectedMax));
            assertThat(havenaskThreadPoolExecutor.getMaximumPoolSize(), equalTo(expectedMax));
        });
