...
import org.havenask.threadpool.Scheduler.ReschedulingRunnable;
 ...
 */
public class ScheduleWithFixedDelayTests extends HavenaskTestCase {
    ...
    public void testThatRunnableIsRescheduled() throws Exception {
        ...
        final Runnable countingRunnable = () -> {
            if (rarely()) {
                throw new HavenaskException("sometimes we throw before counting down");
            }

            latch.countDown();

            if (randomBoolean()) {
                throw new HavenaskException("this shouldn't cause the test to fail!");
            }
        };
        ...
        threadPool = new ThreadPool(Settings.builder().put(Node.NODE_NAME_SETTING.getKey(), "fixed delay tests").build()) {
            ...
            public ScheduledCancellable schedule(Runnable command, TimeValue delay, String executor) {
                if (command instanceof ReschedulingRunnable) {
                    ((ReschedulingRunnable) command).onRejection(new HavenaskRejectedExecutionException());
                } else {
