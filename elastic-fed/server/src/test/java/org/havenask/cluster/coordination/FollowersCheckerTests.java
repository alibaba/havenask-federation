...
import static org.havenask.cluster.coordination.FollowersChecker.FOLLOWER_CHECK_ACTION_NAME;
...
public class FollowersCheckerTests extends HavenaskTestCase {
    ...
    public void testFailsNodeThatRejectsCheck() {
        ...
        testBehaviourOfFailingNode(settings, () -> {
                throw new HavenaskException("simulated exception");
            },
            "followers check retry count exceeded",
            (FOLLOWER_CHECK_RETRY_COUNT_SETTING.get(settings) - 1) * FOLLOWER_CHECK_INTERVAL_SETTING.get(settings).millis(),
            () -> new StatusInfo(HEALTHY, "healthy-info"));
    ...
    public void testResponder() {
...

        {
            ...
            coordinatorException.set(new HavenaskException(exceptionMessage));
