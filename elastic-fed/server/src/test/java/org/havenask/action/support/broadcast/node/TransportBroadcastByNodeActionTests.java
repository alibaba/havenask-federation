...
package org.havenask.action.support.broadcast.node;
...
public class TransportBroadcastByNodeActionTests extends HavenaskTestCase {
    ...
    class TestTransportBroadcastByNodeAction
        ...
        protected EmptyResult shardOperation(Request request, ShardRouting shardRouting) {
            if (rarely()) {
                ...
                HavenaskException e = new HavenaskException("operation failed");
    ...
    public void testOperationExecution() throws Exception {
        ...
        for (Object result : action.getResults().values()) {
            if (!(result instanceof HavenaskException)) {
                successfulShards++;
            } else {
                failedShards++;
            }
