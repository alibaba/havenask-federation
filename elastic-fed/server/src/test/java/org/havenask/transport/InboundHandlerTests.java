...
public class InboundHandlerTests extends HavenaskTestCase {
    ...
    public void testRequestAndResponse() throws Exception {
        ...
        if (isError) {
            ...
            transportChannel.sendResponse(new HavenaskException("boom"));
        } else {
        ...
        if (isError) {
            ...
            assertThat(exceptionCaptor.get().getCause(), instanceOf(HavenaskException.class));
    ...
    public void testSendsErrorResponseToHandshakeFromCompatibleVersion() throws Exception {
        ...
        requestHeader.headers = Tuple.tuple(org.havenask.common.collect.Map.of(), org.havenask.common.collect.Map.of());
        requestHeader.features = org.havenask.common.collect.Set.of();
        handler.inboundMessage(channel, requestMessage);
        ...
        return new InboundMessage(requestHeader, ReleasableBytesReference.wrap(BytesArray.EMPTY), () -> { }) {
            ...
            public StreamInput openOrGetStreamInput() {
                final StreamInput streamInput = new InputStreamStreamInput(new InputStream() {
                    @Override
                    public int read() {
                        throw new HavenaskException("unreadable handshake");
                    }
                });
