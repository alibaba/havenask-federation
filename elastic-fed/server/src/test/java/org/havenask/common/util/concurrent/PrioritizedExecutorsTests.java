...
public class PrioritizedExecutorsTests extends HavenaskTestCase {
    ...
    public void testSubmitPrioritizedExecutorWithRunnables() throws Exception {
        ExecutorService executor =
                HavenaskExecutors.newSinglePrioritizing(getName(), HavenaskExecutors.daemonThreadFactory(getTestName()), holder, null);
    ...
    public void testSubmitPrioritizedExecutorWithMixed() throws Exception {
        ExecutorService executor = HavenaskExecutors.newSinglePrioritizing(getTestName(),
            HavenaskExecutors.daemonThreadFactory(getTestName()), holder, null);
        List<Integer> results = new ArrayList<>(8);
        CountDownLatch awaitingLatch = new CountDownLatch(1);
        CountDownLatch finishedLatch = new CountDownLatch(8);
        executor.submit(new AwaitingJob(awaitingLatch));
    ...
    public void testTimeout() throws Exception {
        ScheduledExecutorService timer = Executors.newSingleThreadScheduledExecutor(HavenaskExecutors.daemonThreadFactory(getTestName()));
        PrioritizedHavenaskThreadPoolExecutor executor =
                HavenaskExecutors.newSinglePrioritizing(getName(), HavenaskExecutors.daemonThreadFactory(getTestName()), holder, timer);
        ...
        invoked.await();
        PrioritizedHavenaskThreadPoolExecutor.Pending[] pending = executor.getPending();
