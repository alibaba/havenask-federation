...
package org.havenask.search.morelikethis;
...
import org.havenask.index.query.MoreLikeThisQueryBuilder.Item;
...
public class MoreLikeThisIT extends HavenaskIntegTestCase {
    ...
    public void testMoreLikeThisWithAliases() throws Exception {
        ...
        client().index(indexRequest("test").type("type1").id("2")
                .source(jsonBuilder().startObject().field("text", "lucene release").endObject())).actionGet();
        client().index(indexRequest("test").type("type1").id("3")
                .source(jsonBuilder().startObject().field("text", "havenask beta").endObject())).actionGet();
        client().index(indexRequest("test").type("type1").id("4")
                .source(jsonBuilder().startObject().field("text", "havenask release").endObject())).actionGet();
        client().admin().indices().refresh(refreshRequest()).actionGet();
    ...
    public void testMoreLikeThisWithAliasesInLikeDocuments() throws Exception {
        ...
        client().index(indexRequest(indexName).type(typeName).id("1")
                .source(jsonBuilder().startObject().field("text", "havenask index").endObject())).actionGet();
    ...
    public void testNumericField() throws Exception {
        ...
        client().prepareIndex("test", "type", "2")
                .setSource(jsonBuilder().startObject().field("string_value", "havenask index").field("int_value", 42).endObject())
                .get();
