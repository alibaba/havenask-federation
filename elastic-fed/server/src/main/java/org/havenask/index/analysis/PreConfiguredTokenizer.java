...
public final class PreConfiguredTokenizer extends PreConfiguredAnalysisComponent<TokenizerFactory> {
    ...
    /**
     * Create a pre-configured tokenizer that may vary based on the Havenask version.
     *
     * @param name the name of the tokenizer in the api
     * @param create builds the tokenizer
     */
    public static PreConfiguredTokenizer havenaskVersion(String name, Function<org.havenask.Version, Tokenizer> create) {
        return new PreConfiguredTokenizer(name, CachingStrategy.HAVENASK, create);
    }
