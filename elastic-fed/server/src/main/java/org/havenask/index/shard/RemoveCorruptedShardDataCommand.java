...
import org.havenask.index.translog.TruncateTranslogAction;
...
public class RemoveCorruptedShardDataCommand extends HavenaskNodeCommand {
    ...
    throws IOException {
        ...
        if (options.has(folderOption)) {
            ...
            if (Files.exists(indexPath) == false || Files.isDirectory(indexPath) == false) {
                throw new HavenaskException("index directory [" + indexPath + "], must exist and be a directory");
            }
            ...
            if (Files.isDirectory(path) && shardIdFileName.chars().allMatch(Character::isDigit) // SHARD-ID path element check
                ...
                throw new HavenaskException("Unable to resolve shard id. Wrong folder structure at [ " + path.toString()
        ...
        if (indexMetadata == null) {
            throw new HavenaskException("Unable to find index in cluster state");
        }
        ...
        throw new HavenaskException("Unable to resolve shard path for index [" + indexMetadata.getIndex().getName() +
    ...
    private static void confirm(String msg, Terminal terminal) {
        ...
        if (text.equalsIgnoreCase("y") == false) {
            throw new HavenaskException("aborted by user");
        }
    }
    ...
    public void processNodePaths(Terminal terminal, Path[] dataPaths, int nodeLockId, OptionSet options, Environment environment)
        ...
        findAndProcessShardPath(options, environment, dataPaths, nodeLockId, clusterState, shardPath -> {
            ...
            if (Files.exists(translogPath) == false || Files.isDirectory(translogPath) == false) {
                throw new HavenaskException("translog directory [" + translogPath + "], must exist and be a directory");
            }
            ...
            try (Directory indexDir = indexDirectory) {
                ...
                try (Lock writeIndexLock = indexDir.obtainLock(IndexWriter.WRITE_LOCK_NAME)) {
                    ...
                    if (indexStatus == CleanStatus.CLEAN && translogStatus == CleanStatus.CLEAN) {
                        throw new HavenaskException("Shard does not seem to be corrupted at " + shardPath.getDataPath()
                    ...
                    if (indexStatus == CleanStatus.UNRECOVERABLE) {
                        ...
                        throw new HavenaskException("Index is unrecoverable");
                    }
                    ...
                    final String msg = "Failed to lock shard's directory at [" + indexPath + "], is Havenask still running?";
                    terminal.println(msg);
                    throw new HavenaskException(msg);
                }
    ...
    private Directory getDirectory(Path indexPath) {
        ...
        try {
            ...
            throw new HavenaskException("ERROR: could not open directory \"" + indexPath + "\"; exiting");
        }
        return directory;
    }
    ...
    private void newAllocationId(ShardPath shardPath, Terminal terminal) throws IOException {
        ...
        if (shardStateMetadata == null) {
            throw new HavenaskException("No shard state meta data at " + shardStatePath);
        }
    ...
    private void printRerouteCommand(ShardPath shardPath, Terminal terminal, boolean allocateStale)
        ...
        if (nodeMetadata == null) {
            throw new HavenaskException("No node meta data at " + nodePath);
        }
    ...
    private Path getNodePath(ShardPath shardPath) {
        ...
        if (Files.exists(nodePath) == false ||
            ...
            throw new HavenaskException("Unable to resolve node path for " + shardPath);
        }
        return nodePath;
    }
