 ...
 * and current merges.
 */
class HavenaskConcurrentMergeScheduler extends ConcurrentMergeScheduler {
    ...
    HavenaskConcurrentMergeScheduler(ShardId shardId, IndexSettings indexSettings) {
    ...
    protected MergeThread getMergeThread(MergeSource mergeSource, MergePolicy.OneMerge merge) throws IOException {
        ...
        thread.setName(HavenaskExecutors.threadName(indexSettings, "[" + shardId.getIndexName() + "][" + shardId.id() + "]: " +
            thread.getName()));
        return thread;
    }
