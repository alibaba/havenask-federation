 ...
 *     <code>Math.max(1, Math.min(4, {@link HavenaskExecutors#allocatedProcessors(Settings)} / 2))</code>
...
public final class MergeSchedulerConfig {
    ...
    public static final Setting<Integer> MAX_THREAD_COUNT_SETTING =
        new Setting<>("index.merge.scheduler.max_thread_count",
            (s) -> Integer.toString(Math.max(1, Math.min(4, HavenaskExecutors.allocatedProcessors(s) / 2))),
