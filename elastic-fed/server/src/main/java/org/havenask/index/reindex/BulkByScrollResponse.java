...
import org.havenask.index.reindex.BulkByScrollTask.Status;
...
public class BulkByScrollResponse extends ActionResponse implements ToXContentFragment {
    ...
    private static Object parseFailure(XContentParser parser) throws IOException {
       ...
       String nodeId = null;
       HavenaskException bulkExc = null;
       HavenaskException searchExc = null;
           ...
           if (token == Token.START_ARRAY) {
               ...
               switch (name) {
                   case SearchFailure.REASON_FIELD:
                       searchExc = HavenaskException.fromXContent(parser);
                       break;
                   case Failure.CAUSE_FIELD:
                       bulkExc = HavenaskException.fromXContent(parser);
                       break;
                   default:
                       parser.skipChildren();
               }
       ...
       if (bulkExc != null) {
           ...
           throw new HavenaskParseException("failed to parse failures array. At least one of {reason,cause} must be present");
       }
    }
