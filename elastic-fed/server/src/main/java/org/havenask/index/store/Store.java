...
import org.havenask.common.util.concurrent.RefCounted;
...
import org.havenask.index.engine.CombinedDeletionPolicy;
import org.havenask.index.engine.Engine;
import org.havenask.index.seqno.SequenceNumbers;
import org.havenask.index.shard.AbstractIndexShardComponent;
...
/**
 * A Store provides plain access to files written by an havenask index shard. Each shard
...
public class Store extends AbstractIndexShardComponent implements Closeable, RefCounted {
    ...
    public static final class MetadataSnapshot implements Iterable<StoreFileMetadata>, Writeable {
        ...
        static LoadedMetadata loadMetadata(IndexCommit commit, Directory directory, Logger logger) throws IOException {
            ...
            try {
                ...
                if (maxVersion == null) {
                    maxVersion = org.havenask.Version.CURRENT.minimumIndexCompatibilityVersion().luceneVersion;
                }
     ...
     *
     * @see MetadataSnapshot#recoveryDiff(org.havenask.index.store.Store.MetadataSnapshot)
     */
    ...
    public interface OnClose extends Consumer<ShardLock> {
        OnClose EMPTY = new OnClose() {
            /**
             * This method is called while the provided {@link org.havenask.env.ShardLock} is held.
    ...
    public void trimUnsafeCommits(final long lastSyncedGlobalCheckpoint, final long minRetainedTranslogGen,
                                  final org.havenask.Version indexVersionCreated) throws IOException {
        metadataLock.writeLock().lock();
        try {
    ...
    /**
     * Returns a {@link org.havenask.index.seqno.SequenceNumbers.CommitInfo} of the safe commit if exists.
     */
