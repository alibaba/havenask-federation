...
import static org.havenask.search.fetch.subphase.InnerHitsContext.intersect;
...
public class NestedQueryBuilder extends AbstractQueryBuilder<NestedQueryBuilder> {
    ...
    public static String scoreModeAsString(ScoreMode scoreMode) {
        if (scoreMode == ScoreMode.Total) {
            // Lucene uses 'total' but 'sum' is more consistent with other havenask APIs
            return "sum";
        } else {
    ...
    protected Query doToQuery(QueryShardContext context) throws IOException {
        if (context.allowExpensiveQueries() == false) {
            throw new HavenaskException("[joining] queries cannot be executed when '" +
        ...
        return new HavenaskToParentBlockJoinQuery(innerQuery, parentFilter, scoreMode,
