...
package org.havenask.action.admin.indices.analyze;
...
import org.havenask.index.analysis.AnalyzerComponents;
import org.havenask.index.analysis.AnalyzerComponentsProvider;
import org.havenask.index.analysis.CharFilterFactory;
import org.havenask.index.analysis.NameOrDefinition;
    ...
    private static List<AnalyzeAction.AnalyzeToken> simpleAnalyze(AnalyzeAction.Request request,
        ...
        for (String text : request.text()) {
            try (TokenStream stream = analyzer.tokenStream("", text)) {
                ...
                throw new HavenaskException("failed to analyze", e);
            }
        }
        return tokens;
    }
    ...
    private static String writeCharStream(Reader input) {
        ...
        do {
            try {
                ...
                throw new HavenaskException("failed to analyze (charFiltering)", e);
            }
            if (len > 0) {
                sb.append(buf, 0, len);
            }
