...
public class IndicesOptions implements ToXContentFragment {
    ...
    public static IndicesOptions fromXContent(XContentParser parser) throws IOException {
        ...
        if (token != Token.START_OBJECT) {
            throw new HavenaskParseException("expected START_OBJECT as the token but was " + token);
        }
        while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
            if (token == XContentParser.Token.FIELD_NAME) {
                    ...
                    if (wildcardStates == null) {
                        ...
                        while ((token = parser.nextToken()) != Token.END_ARRAY) {
                            if (token.isValue()) {
                                ...
                                throw new HavenaskParseException("expected values within array for " +
                                    EXPAND_WILDCARDS_FIELD.getPreferredName());
                            }
                        }
                    } else {
                        throw new HavenaskParseException("already parsed expand_wildcards");
                    }
                } else {
                    throw new HavenaskParseException(EXPAND_WILDCARDS_FIELD.getPreferredName() +
                ...
                if (EXPAND_WILDCARDS_FIELD.match(currentFieldName, parser.getDeprecationHandler())) {
                    ...
                    throw new HavenaskParseException("could not read indices options. unexpected index option [" +
                        currentFieldName + "]");
                }
            } else {
                throw new HavenaskParseException("could not read indices options. unexpected object field [" +
                    currentFieldName + "]");
            }
        }

        if (wildcardStates == null) {
            throw new HavenaskParseException("indices options xcontent did not contain " + EXPAND_WILDCARDS_FIELD.getPreferredName());
        }
        if (ignoreUnavailable == null) {
            throw new HavenaskParseException("indices options xcontent did not contain " +
