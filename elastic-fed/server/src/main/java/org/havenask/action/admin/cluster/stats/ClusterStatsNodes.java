...
public class ClusterStatsNodes implements ToXContentFragment {
    ...
    public static class OsStats implements ToXContentFragment {
        ...
        final org.havenask.monitor.os.OsStats.Mem mem;
        ...
        private OsStats(List<NodeInfo> nodeInfos, List<NodeStats> nodeStatsList) {
            ...
            this.mem = new org.havenask.monitor.os.OsStats.Mem(totalMemory, freeMemory);
        }
        ...
        public org.havenask.monitor.os.OsStats.Mem getMem() {
            return mem;
        }
    ...
    public static class JvmStats implements ToXContentFragment {
        ...
        private JvmStats(List<NodeInfo> nodeInfos, List<NodeStats> nodeStatsList) {
            ...
            for (NodeStats nodeStats : nodeStatsList) {
                org.havenask.monitor.jvm.JvmStats js = nodeStats.getJvm();
                if (js == null) {
                    continue;
                }
            ...
            for (NodeStats nodeStat : nodeStats) {
                if (nodeStat.getIngestStats() != null) {
                    for (Map.Entry<String,
                            List<org.havenask.ingest.IngestStats.ProcessorStat>> processorStats : nodeStat.getIngestStats()
                            .getProcessorStats().entrySet()) {
                        pipelineIds.add(processorStats.getKey());
                        for (org.havenask.ingest.IngestStats.ProcessorStat stat : processorStats.getValue()) {
                            stats.compute(stat.getType(), (k, v) -> {
                                org.havenask.ingest.IngestStats.Stats nodeIngestStats = stat.getStats();
                                if (v == null) {
                                    return new long[] {
