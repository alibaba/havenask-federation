...
package org.havenask.action.admin.cluster.node.tasks.list;

import org.havenask.HavenaskException;
import org.havenask.action.TaskOperationFailure;
import org.havenask.action.support.tasks.BaseTasksResponse;
    ...
    protected static <T> ConstructingObjectParser<T, Void> setupParser(String name,
        ...
        ConstructingObjectParser<T, Void> parser = new ConstructingObjectParser<>(name, true,
            constructingObjects -> {
                ...
                @SuppressWarnings("unchecked")
                List<HavenaskException> nodeFailures = (List<HavenaskException>) constructingObjects[i];
        ...
        parser.declareObjectArray(optionalConstructorArg(),
            (p, c) -> HavenaskException.fromXContent(p), new ParseField(NODE_FAILURES));
        return parser;
    }
