                ...
                if (token == Token.FIELD_NAME) {
                    ...
                    if (INDEX.match(currentFieldName, parser.getDeprecationHandler())) {
                        ...
                        if (parser.isBooleanValue()) {
                            ...
                            throw new HavenaskParseException("illegal type for _source: [{}]", token);
                        }
                    } else {
                        throw new HavenaskParseException("failed to parse multi get request. unknown field [{}]", currentFieldName);
                    }
                } else if (token == Token.START_ARRAY) {
                        ...
                        while ((token = parser.nextToken()) != Token.END_OBJECT) {
                            if (token == Token.FIELD_NAME) {
                                ...
                                if ("includes".equals(currentFieldName) || "include".equals(currentFieldName)) {
                                    ...
                                    throw new HavenaskParseException("source definition may not contain [{}]", parser.text());
                                }
                            } else if (token == Token.START_ARRAY) {
                                while ((token = parser.nextToken()) != Token.END_ARRAY) {
                                ...
                                throw new HavenaskParseException("unexpected token while parsing source settings");
                            }
                        }
