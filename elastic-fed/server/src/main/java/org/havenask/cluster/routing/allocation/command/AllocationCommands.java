...
public class AllocationCommands implements ToXContentFragment {
     ...
     * @throws HavenaskException if something happens during execution
     */
    ...
    public static AllocationCommands fromXContent(XContentParser parser) throws IOException {
        ...
        if (token == null) {
            throw new HavenaskParseException("No commands");
        }
        if (token == XContentParser.Token.FIELD_NAME) {
            if (!parser.currentName().equals("commands")) {
                throw new HavenaskParseException("expected field name to be named [commands], got [{}] instead", parser.currentName());
            }
            token = parser.nextToken();
            if (token != XContentParser.Token.START_ARRAY) {
                throw new HavenaskParseException("commands should follow with an array element");
            }
        } else if (token == XContentParser.Token.START_ARRAY) {
            // ok...
        } else {
            throw new HavenaskParseException("expected either field name [commands], or start array, got [{}] instead", token);
        }
        while ((token = parser.nextToken()) != XContentParser.Token.END_ARRAY) {
            if (token == XContentParser.Token.START_OBJECT) {
                ...
                if (parser.nextToken() != XContentParser.Token.END_OBJECT) {
                    throw new HavenaskParseException("allocation command is malformed, done parsing a command," +
                        " but didn't get END_OBJECT, got [{}] instead", token);
                }
            } else {
                throw new HavenaskParseException("allocation command is malformed, got [{}] instead", token);
            }
        }
        return commands;
    }
