        ...
        while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                if ("index".equals(currentFieldName)) {
                    ...
                    throw new HavenaskParseException("[{}] command does not support field [{}]", NAME, currentFieldName);
                }
            } else {
                throw new HavenaskParseException("[{}] command does not support complex json tokens [{}]", NAME, token);
            }
        }
        if (index == null) {
            throw new HavenaskParseException("[{}] command missing the index parameter", NAME);
        }
        if (shardId == -1) {
            throw new HavenaskParseException("[{}] command missing the shard parameter", NAME);
        }
        if (fromNode == null) {
            throw new HavenaskParseException("[{}] command missing the from_node parameter", NAME);
        }
        if (toNode == null) {
            throw new HavenaskParseException("[{}] command missing the to_node parameter", NAME);
        }
