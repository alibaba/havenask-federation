...
package org.havenask.monitor.jvm;
...
public class HotThreads {
    ...
    private static boolean isIdleThread(ThreadInfo threadInfo) {
        ...
        for (StackTraceElement frame : threadInfo.getStackTrace()) {
            ...
            if (className.equals("org.havenask.threadpool.ThreadPool$CachedTimeThread") &&
                methodName.equals("run")) {
                return true;
            }
            if (className.equals("org.havenask.indices.ttl.IndicesTTLService$Notifier") &&
    ...
    private String innerDetect() throws Exception {
        ...
        if (threadBean.isThreadCpuTimeSupported() == false) {
            throw new HavenaskException("thread CPU time is not supported on this JDK");
        }
