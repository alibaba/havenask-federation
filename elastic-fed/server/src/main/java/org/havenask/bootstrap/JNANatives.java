...
import static org.havenask.bootstrap.JNAKernel32Library.SizeT;
...
class JNANatives {
    ...
    // otherwise they are only inherited for new threads (Havenask app threads)
    static boolean LOCAL_SYSTEM_CALL_FILTER_ALL = false;
    // set to the maximum number of threads that can be created for
    // the user ID that owns the running Havenask process
