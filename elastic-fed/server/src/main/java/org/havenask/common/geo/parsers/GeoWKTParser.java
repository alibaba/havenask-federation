...
public class GeoWKTParser {
    ...
    private static ShapeBuilder parseGeometry(StreamTokenizer stream, GeoShapeType shapeType, final boolean ignoreZValue,
        ...
        if (shapeType != null && shapeType != GeoShapeType.GEOMETRYCOLLECTION) {
            if (type.wktName().equals(shapeType.wktName()) == false) {
                throw new HavenaskParseException("Expected geometry type [{}] but found [{}]", shapeType, type);
            }
        }
        switch (type) {
            case POINT:
    ...
    private static EnvelopeBuilder parseBBox(StreamTokenizer stream) throws IOException, HavenaskParseException {
    ...
    private static List<Coordinate> parseCoordinateList(StreamTokenizer stream, final boolean ignoreZValue, final boolean coerce)
        ...
        if (isOpenParen && nextCloser(stream).equals(RPAREN) == false) {
            throw new HavenaskParseException("expected: [{}]" + RPAREN + " but found: [{}]" + tokenString(stream), stream.lineno());
        }
        ...
        if (coordinates.size() >= coordinatesNeeded) {
            if (!coordinates.get(0).equals(coordinates.get(coordinates.size() - 1))) {
                if (coerce) {
                    ...
                    throw new HavenaskParseException("invalid LinearRing found (coordinates are not closed)");
                }
            }
        }
        if (coordinates.size() < 4) {
            throw new HavenaskParseException("invalid number of points in LinearRing (found [{}] - must be >= 4)",
                coordinates.size());
        }
    ...
    private static String nextWord(StreamTokenizer stream) throws HavenaskParseException, IOException {
        ...
        throw new HavenaskParseException("expected word but found: " + tokenString(stream), stream.lineno());
    }

    private static double nextNumber(StreamTokenizer stream) throws IOException, HavenaskParseException {
        if (stream.nextToken() == StreamTokenizer.TT_WORD) {
            if (stream.sval.equalsIgnoreCase(NAN)) {
                ...
                try {
                    ...
                    throw new HavenaskParseException("invalid number found: " + stream.sval, stream.lineno());
                }
            }
        }
        throw new HavenaskParseException("expected number but found: " + tokenString(stream), stream.lineno());
    }
    ...
    private static String nextEmptyOrOpen(StreamTokenizer stream) throws IOException, HavenaskParseException {
        final String next = nextWord(stream);
        if (next.equals(EMPTY) || next.equals(LPAREN)) {
            return next;
        }
        throw new HavenaskParseException("expected " + EMPTY + " or " + LPAREN
            + " but found: " + tokenString(stream), stream.lineno());
    }

    private static String nextCloser(StreamTokenizer stream) throws IOException, HavenaskParseException {
    ...
    private static String nextComma(StreamTokenizer stream) throws IOException, HavenaskParseException {
        if (nextWord(stream).equals(COMMA)) {
            return COMMA;
        }
        throw new HavenaskParseException("expected " + COMMA + " but found: " + tokenString(stream), stream.lineno());
    }

    private static String nextCloserOrComma(StreamTokenizer stream) throws IOException, HavenaskParseException {
        String token = nextWord(stream);
        if (token.equals(COMMA) || token.equals(RPAREN)) {
            return token;
        }
        throw new HavenaskParseException("expected " + COMMA + " or " + RPAREN
            + " but found: " + tokenString(stream), stream.lineno());
    }

    /** next word in the stream */
    private static void checkEOF(StreamTokenizer stream) throws HavenaskParseException, IOException {
        if (stream.nextToken() != StreamTokenizer.TT_EOF) {
            throw new HavenaskParseException("expected end of WKT string but found additional text: "
