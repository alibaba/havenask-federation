...
package org.havenask.common.time;
...
public class JavaDateMathParser implements DateMathParser {
    ...
    public Instant parse(String text, LongSupplier now, boolean roundUpProperty, ZoneId timeZone) {
        ...
        if (text.startsWith("now")) {
            try {
                ...
                throw new HavenaskParseException("could not read the current timestamp", e);
            }
    ...
    private Instant parseMath(final String mathString, final Instant time, final boolean roundUpProperty,
                           ZoneId timeZone) throws HavenaskParseException {
        if (timeZone == null) {
        ...
        for (int i = 0; i < mathString.length(); ) {
            ...
            if (c == '/') {
                ...
                if (c == '+') {
                    ...
                    throw new HavenaskParseException("operator not supported for date math [{}]", mathString);
                }
            }

            if (i >= mathString.length()) {
                throw new HavenaskParseException("truncated date math [{}]", mathString);
            }
            ...
            if (round) {
                if (num != 1) {
                    throw new HavenaskParseException("rounding `/` can only be used on single unit types [{}]", mathString);
                }
            }
            ...
            switch (unit) {
                ...
                case 's':
                    ...
                    break;
                default:
                    throw new HavenaskParseException("unit [{}] not supported for date math [{}]", unit, mathString);
            }
    ...
    private Instant parseDateTime(String value, ZoneId timeZone, boolean roundUpIfNoTime) {
        if (Strings.isNullOrEmpty(value)) {
            throw new HavenaskParseException("cannot parse empty date");
        }
        ...
        try {
            ...
            throw new HavenaskParseException("failed to parse date field [{}] with format [{}]: [{}]",
