...
import org.havenask.geometry.utils.StandardValidator;
...
public class GeoBoundingBox implements ToXContentObject, Writeable {
     ...
     */
    public static GeoBoundingBox parseBoundingBox(XContentParser parser) throws IOException, HavenaskParseException {
        XContentParser.Token token = parser.currentToken();
        if (token != XContentParser.Token.START_OBJECT) {
            throw new HavenaskParseException("failed to parse bounding box. Expected start object but found [{}]", token);
        }
                ...
                if (WKT_FIELD.match(currentFieldName, parser.getDeprecationHandler())) {
                    try {
                        ...
                        if (ShapeType.ENVELOPE.equals(geometry.type()) == false) {
                            throw new HavenaskParseException("failed to parse WKT bounding box. ["
                                + geometry.type() + "] found. expected [" + ShapeType.ENVELOPE + "]");
                        }
                        envelope = (Rectangle) geometry;
                    } catch (ParseException|IllegalArgumentException e) {
                        throw new HavenaskParseException("failed to parse WKT bounding box", e);
                    }
                    ...
                    if (TOP_LEFT_FIELD.match(currentFieldName, parser.getDeprecationHandler())) {
                        ...
                        throw new HavenaskParseException("failed to parse bounding box. unexpected field [{}]", currentFieldName);
                    }
                }
            } else {
                throw new HavenaskParseException("failed to parse bounding box. field name expected but [{}] found", token);
            }
        }
        if (envelope != null) {
            if (Double.isNaN(top) == false || Double.isNaN(bottom) == false || Double.isNaN(left) == false ||
                Double.isNaN(right) == false) {
                throw new HavenaskParseException("failed to parse bounding box. Conflicting definition found "
