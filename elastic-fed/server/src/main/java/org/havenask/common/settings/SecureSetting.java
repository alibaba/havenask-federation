 ...
 * A secure setting.
 *
 * This class allows access to settings from the Havenask keystore.
 */
public abstract class SecureSetting<T> extends Setting<T> {
    ...
    private static final boolean ALLOW_INSECURE_SETTINGS =
        Booleans.parseBoolean(System.getProperty("havenask.allow_insecure_settings", "false"));
        ...
        if (secureSettings == null || secureSettings.getSettingNames().contains(getKey()) == false) {
            if (super.exists(settings)) {
                throw new IllegalArgumentException("Setting [" + getKey() + "] is a secure setting" +
                    " and must be stored inside the Havenask keystore, but was found inside havenask.yml");
            }
