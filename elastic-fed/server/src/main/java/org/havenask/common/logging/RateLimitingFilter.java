...
import static org.havenask.common.logging.DeprecatedMessage.X_OPAQUE_ID_FIELD_NAME;
...
public class RateLimitingFilter extends AbstractFilter {
    ...
    public Result filter(Message message) {
        if (message instanceof HavenaskLogMessage) {
            final HavenaskLogMessage havenaskLogMessage = (HavenaskLogMessage) message;

            String xOpaqueId = havenaskLogMessage.getValueFor(X_OPAQUE_ID_FIELD_NAME);
            final String key = havenaskLogMessage.getValueFor("key");
