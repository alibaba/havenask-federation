...
public class HavenaskExecutors {

    private static final DeprecationLogger deprecationLogger = DeprecationLogger.getLogger(HavenaskExecutors.class);
     ...
     * available to Havenask (e.g., because of CPU limits).
     */
    ...
    public static PrioritizedHavenaskThreadPoolExecutor newSinglePrioritizing(String name, ThreadFactory threadFactory,
                                                                                ThreadContext contextHolder,
                                                                                ScheduledExecutorService timer) {
        return new PrioritizedHavenaskThreadPoolExecutor(name, 1, 1, 0L, TimeUnit.MILLISECONDS, threadFactory, contextHolder, timer);
    }

    public static HavenaskThreadPoolExecutor newScaling(String name, int min, int max, long keepAliveTime, TimeUnit unit,
                                                          ThreadFactory threadFactory, ThreadContext contextHolder) {
        ExecutorScalingQueue<Runnable> queue = new ExecutorScalingQueue<>();
        HavenaskThreadPoolExecutor executor =
            new HavenaskThreadPoolExecutor(name, min, max, keepAliveTime, unit, queue, threadFactory,
                new ForceQueuePolicy(), contextHolder);
        queue.executor = executor;
        return executor;
    }

    public static HavenaskThreadPoolExecutor newFixed(String name, int size, int queueCapacity,
                                                        ThreadFactory threadFactory, ThreadContext contextHolder) {
        ...
        return new HavenaskThreadPoolExecutor(name, size, size, 0, TimeUnit.MILLISECONDS,
            queue, threadFactory, new HavenaskAbortPolicy(), contextHolder);
    }
     ...
     */
    public static HavenaskThreadPoolExecutor newAutoQueueFixed(String name, int size, int initialQueueCapacity, int minQueueSize,
        ...
        return new QueueResizingHavenaskThreadPoolExecutor(name, size, size, 0, TimeUnit.MILLISECONDS,
                queue, minQueueSize, maxQueueSize, TimedRunnable::new, frameSize, targetedResponseTime, threadFactory,
                new HavenaskAbortPolicy(), contextHolder);
    }
    ...
    private static final class DirectExecutorService extends AbstractExecutorService {
        ...
        @SuppressForbidden(reason = "properly rethrowing errors, see HavenaskExecutors.rethrowErrors")
    ...
    public static String threadName(final String nodeName, final String namePrefix) {
        ...
        return "havenask" + (nodeName.isEmpty() ? "" : "[") + nodeName + (nodeName.isEmpty() ? "" : "]") + "[" + namePrefix + "]";
    }
    ...
    public static ThreadFactory daemonThreadFactory(String namePrefix) {
        return new HavenaskThreadFactory(namePrefix);
    }

    static class HavenaskThreadFactory implements ThreadFactory {

        final ThreadGroup group;
        final AtomicInteger threadNumber = new AtomicInteger(1);
        final String namePrefix;

        HavenaskThreadFactory(String namePrefix) {
