...
import org.havenask.common.util.HavenaskSloppyMath;
...
import static org.havenask.common.geo.GeoUtils.normalizeLat;
import static org.havenask.common.geo.GeoUtils.normalizeLon;
...
public final class GeoTileUtils {
     ...
     */
    static int parsePrecision(XContentParser parser) throws IOException, HavenaskParseException {
    ...
    public static Rectangle toBoundingBox(int xTile, int yTile, int precision) {
        ...
        final double minY = Math.toDegrees(HavenaskSloppyMath.atan(HavenaskSloppyMath.sinh(minN)));
        final double minX = ((xTile) / tiles * 360.0) - 180;
        final double maxY = Math.toDegrees(HavenaskSloppyMath.atan(HavenaskSloppyMath.sinh(maxN)));
    ...
    private static GeoPoint zxyToGeoPoint(int zoom, int xTile, int yTile) {
        ...
        final double lat = Math.toDegrees(HavenaskSloppyMath.atan(HavenaskSloppyMath.sinh(n)));
