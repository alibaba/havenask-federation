...
public class IncludeExclude implements Writeable, ToXContentFragment {
    ...
    private static Set<BytesRef> parseArrayToSet(XContentParser parser) throws IOException {
        ...
        if (parser.currentToken() != XContentParser.Token.START_ARRAY) {
            throw new HavenaskParseException("Missing start of array in include/exclude clause");
        }
        while (parser.nextToken() != XContentParser.Token.END_ARRAY) {
            if (!parser.currentToken().isValue()) {
                throw new HavenaskParseException("Array elements in include/exclude clauses should be string values");
            }
