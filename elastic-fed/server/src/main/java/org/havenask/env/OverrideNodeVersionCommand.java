...
public class OverrideNodeVersionCommand extends HavenaskNodeCommand {
    private static final String TOO_NEW_MESSAGE =
        DELIMITER +
            "\n" +
            "This data path was last written by Havenask version [V_NEW] and may no\n" +
            "longer be compatible with Havenask version [V_CUR]. This tool will bypass\n" +
    ...
    private static final String TOO_OLD_MESSAGE =
        DELIMITER +
            "\n" +
            "This data path was last written by Havenask version [V_OLD] which may be\n" +
            "too old to be readable by Havenask version [V_CUR].  This tool will bypass\n" +
            "this compatibility check, allowing a version [V_CUR] node to start on this data\n" +
            ...
            "[V_CUR] using one or more intermediate versions of Havenask.\n" +
            "\n" +
            "Do you want to proceed?\n";
    ...
    protected void processNodePaths(Terminal terminal, Path[] dataPaths, int nodeLockId, OptionSet options, Environment env)
        ...
        if (nodeMetadata == null) {
            throw new HavenaskException(NO_METADATA_MESSAGE);
        }

        try {
            nodeMetadata.upgradeToCurrentVersion();
            throw new HavenaskException("found [" + nodeMetadata + "] which is compatible with current version [" + Version.CURRENT
