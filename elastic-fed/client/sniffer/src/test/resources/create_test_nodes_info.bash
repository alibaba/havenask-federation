...
# we want to add a new version of Havenask or configure the
...
# v_nodes_http.json files I won't have to reconfigure Havenask
...
function do_version() {
    ...
    tar xf ../havenask-${version}.tar.gz
    local http_port=9200
    for node in ${nodes}; do
        mkdir ${node}
        cp -r havenask-${version}/* ${node}
        ...
        cat >> ${node}/config/havenask.yml << __ES_YML
node.name:          ${node}
node.master:        ${master}
node.data:          ${data}
...
__ES_YML
        ...
        ${node}/bin/havenask -d -p ${node}/pidfile
