...
import org.havenask.client.sniff.HavenaskNodesSniffer.Scheme;
 ...
 * versions of Havenask.
 */
public class HavenaskNodesSnifferParseTests extends RestClientTestCase {
    ...
    private void checkFile(String file, Node... expected) throws IOException {
        ...
        try {
            ...
            List<Node> nodes = HavenaskNodesSniffer.readHosts(entity, Scheme.HTTP, new JsonFactory());
            /*
    ...
    public void testParsingPublishAddressWithES7Format() throws IOException {
        ...
        assertEquals("havenask.test", nodes.get(0).getHost().getHostName());
        assertEquals(9200, nodes.get(0).getHost().getPort());
        assertEquals("http", nodes.get(0).getHost().getSchemeName());
    }
