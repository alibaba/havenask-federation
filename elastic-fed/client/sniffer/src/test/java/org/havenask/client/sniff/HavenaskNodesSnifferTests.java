...
import org.havenask.client.RestClientTestCase;
...
public class HavenaskNodesSnifferTests extends RestClientTestCase {

    private int sniffRequestTimeout;
    private HavenaskNodesSniffer.Scheme scheme;
    ...
    public void startHttpServer() throws IOException {
        ...
        this.scheme = RandomPicks.randomFrom(getRandom(), HavenaskNodesSniffer.Scheme.values());
        if (rarely()) {
    ...
    public void testConstructorValidation() throws IOException {
        try {
            new HavenaskNodesSniffer(null, 1, HavenaskNodesSniffer.Scheme.HTTP);
        ...
        try (RestClient restClient = RestClient.builder(httpHost).build()) {
            try {
                new HavenaskNodesSniffer(restClient, 1, null);
                fail("should have failed");
            } catch (NullPointerException e) {
                assertEquals(e.getMessage(), "scheme cannot be null");
            }
            try {
                new HavenaskNodesSniffer(restClient, RandomNumbers.randomIntBetween(getRandom(), Integer.MIN_VALUE, 0),
                        HavenaskNodesSniffer.Scheme.HTTP);
                fail("should have failed");
    ...
    public void testSniffNodes() throws IOException {
        HttpHost httpHost = new HttpHost(httpServer.getAddress().getHostString(), httpServer.getAddress().getPort());
        try (RestClient restClient = RestClient.builder(httpHost).build()) {
            HavenaskNodesSniffer sniffer = new HavenaskNodesSniffer(restClient, sniffRequestTimeout, scheme);
            try {
    ...
    private static SniffResponse buildSniffResponse(HavenaskNodesSniffer.Scheme scheme) throws IOException {
        ...
        if (getRandom().nextBoolean()) {
            generator.writeStringField("cluster_name", "havenask");
        }
        if (getRandom().nextBoolean()) {
