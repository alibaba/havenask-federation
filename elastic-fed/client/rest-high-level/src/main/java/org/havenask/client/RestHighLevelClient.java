...
import org.havenask.client.core.CountRequest;
import org.havenask.client.core.CountResponse;
import org.havenask.client.core.GetSourceRequest;
import org.havenask.client.core.GetSourceResponse;
import org.havenask.client.core.MainRequest;
import org.havenask.client.core.MainResponse;
import org.havenask.client.core.MultiTermVectorsRequest;
import org.havenask.client.core.MultiTermVectorsResponse;
import org.havenask.client.core.TermVectorsRequest;
import org.havenask.client.core.TermVectorsResponse;
...
import org.havenask.index.rankeval.RankEvalRequest;
import org.havenask.index.rankeval.RankEvalResponse;
import org.havenask.index.reindex.BulkByScrollResponse;
import org.havenask.index.reindex.DeleteByQueryRequest;
...
import org.havenask.script.mustache.MultiSearchTemplateRequest;
import org.havenask.script.mustache.MultiSearchTemplateResponse;
import org.havenask.script.mustache.SearchTemplateRequest;
import org.havenask.script.mustache.SearchTemplateResponse;
 ...
 * <p>
 *
 * This class can also be sub-classed to expose additional client methods that make use of endpoints added to Havenask through plugins,
 * or to add support for custom response sections, again added to Havenask through plugins.
 * <p>
 *
 ...
 * failure scenarios. Most of the blocking calls can throw an {@link IOException} or an unchecked {@link HavenaskException} in the
 * following cases:
 *
 * <ul>
 ...
 * times out or similar cases where there is no response coming back from the Havenask server</li>
 * <li>an {@link HavenaskException} is usually thrown in case where the server returns a 4xx or 5xx error code. The high-level client
 * then tries to parse the response body error details into a generic HavenaskException and suppresses the original
...
public class RestHighLevelClient implements Closeable {
    ...
    /**
     * Creates a {@link RestHighLevelClient} given the low level {@link RestClientBuilder} that allows to build the
     * {@link RestClient} to be used to perform requests and parsers for custom response sections added to Havenask through plugins.
     */
     ...
     * a list of entries that allow to parse custom response sections added to Havenask through plugins.
     ...
     * Also subclasses can provide parsers for custom response sections added to Havenask through plugins.
     */
    ...
    /**
     * Pings the remote Havenask cluster and returns true if the ping succeeded, false otherwise
     * @param options the request options (e.g. headers), use {@link RequestOptions#DEFAULT} if nothing needs to be customized
    ...
    /**
     * Converts a {@link ResponseException} obtained from the low level REST client into an {@link HavenaskException}.
     * If a response body was returned, tries to parse it as an error returned from Havenask.
     * If no response body was returned or anything goes wrong while parsing the error, returns a new {@link HavenaskStatusException}
     * that wraps the original {@link ResponseException}. The potential exception obtained while parsing is added to the returned
     * exception as a suppressed exception. This method is guaranteed to not throw any exception eventually thrown while parsing.
     */
    protected final HavenaskStatusException parseResponseException(ResponseException responseException) {
        Response response = responseException.getResponse();
        HttpEntity entity = response.getEntity();
        HavenaskStatusException havenaskException;
        RestStatus restStatus = RestStatus.fromCode(response.getStatusLine().getStatusCode());

        if (entity == null) {
            havenaskException = new HavenaskStatusException(
                    responseException.getMessage(), restStatus, responseException);
        } else {
            try {
                havenaskException = parseEntity(entity, BytesRestResponse::errorFromXContent);
                havenaskException.addSuppressed(responseException);
            } catch (Exception e) {
                havenaskException = new HavenaskStatusException("Unable to parse response body", restStatus, responseException);
                havenaskException.addSuppressed(e);
            }
        }
    ...
    protected final <Resp> Resp parseEntity(final HttpEntity entity,
        ...
        if (entity.getContentType() == null) {
            throw new IllegalStateException("Havenask didn't return the [Content-Type] header, unable to parse response body");
        }
     ...
     * used to parse responses from Havenask. Any deprecation warnings
     * emitted there just mean that you are talking to an old version of
     * Havenask. There isn't anything you can do about the deprecation.
     */
