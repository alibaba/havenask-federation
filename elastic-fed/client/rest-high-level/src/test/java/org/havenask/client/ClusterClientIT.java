...
import org.havenask.client.cluster.RemoteConnectionInfo;
import org.havenask.client.cluster.RemoteInfoRequest;
import org.havenask.client.cluster.RemoteInfoResponse;
import org.havenask.client.cluster.SniffModeInfo;
import org.havenask.client.indices.ComponentTemplatesExistRequest;
import org.havenask.client.indices.DeleteComponentTemplateRequest;
import org.havenask.client.indices.GetComponentTemplatesRequest;
import org.havenask.client.indices.GetComponentTemplatesResponse;
import org.havenask.client.indices.PutComponentTemplateRequest;
...
public class ClusterClientIT extends HavenaskRestHighLevelClientTestCase {
    ...
    public void testClusterUpdateSettingNonExistent() {
        ...
        HavenaskException exception = expectThrows(HavenaskException.class, () -> execute(clusterUpdateSettingsRequest,
                highLevelClient().cluster()::putSettings, highLevelClient().cluster()::putSettingsAsync));
        assertThat(exception.status(), equalTo(RestStatus.BAD_REQUEST));
        assertThat(exception.getMessage(), equalTo(
                "Havenask exception [type=illegal_argument_exception, reason=transient setting [" + setting + "], not recognized]"));
    }
