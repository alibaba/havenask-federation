...
2. Install it on the target host with `bin/havenask-plugin install file:///full/path/to/client-benchmark-noop-api-plugin.zip`.
3. Start Havenask on the target host (ideally *not* on the machine
...
Download benchmark data from http://benchmarks.havenask.org.s3.amazonaws.com/corpora/geonames and decompress them.
...
* Benchmark target host IP (the host where Havenask is running)
