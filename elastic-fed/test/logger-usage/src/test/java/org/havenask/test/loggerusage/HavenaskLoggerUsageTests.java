...
package org.havenask.test.loggerusage;
...
import org.havenask.common.SuppressLoggerChecks;
import org.havenask.common.logging.DeprecationLogger;
import org.havenask.test.HavenaskTestCase;
import org.havenask.test.loggerusage.HavenaskLoggerUsageChecker.WrongLoggerUsage;
...
public class HavenaskLoggerUsageTests extends HavenaskTestCase {
        ...
        for (Method method : getClass().getMethods()) {
            if (method.getDeclaringClass().equals(getClass())) {
                if (method.getName().startsWith("check")) {
                    ...
                    HavenaskLoggerUsageChecker.check(errors::add, classInputStream,
    ...
    public void testLoggerUsageCheckerCompatibilityWithLog4j2Logger() throws NoSuchMethodException {
        for (Method method : Logger.class.getMethods()) {
            if (HavenaskLoggerUsageChecker.LOGGER_METHODS.contains(method.getName())) {
        ...
        for (String methodName : HavenaskLoggerUsageChecker.LOGGER_METHODS) {
    ...
    public void checkDeprecationLogger() {
        DeprecationLogger deprecationLogger = DeprecationLogger.getLogger(HavenaskLoggerUsageTests.class);
