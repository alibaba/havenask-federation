...
/**
 * Base test cluster that exposes the basis to run tests against any havenask cluster, whose layout
...
public abstract class TestCluster implements Closeable {
    ...
    public void wipeAllDataStreams() {
        // Feature flag may not be enabled in all gradle modules that use HavenaskIntegTestCase
        if (size() > 0) {
            AcknowledgedResponse response =
                client().admin().indices().deleteDataStream(new DeleteDataStreamAction.Request(new String[]{"*"})).actionGet();
            HavenaskAssertions.assertAcked(response);
        }
    }
    ...
    public void wipeIndices(String... indices) {
        ...
        if (size() > 0) {
            try {
                ...
                HavenaskAssertions.assertAcked(client().admin().indices().prepareDelete(indices)
                ...
                if ("_all".equals(indices[0])) {
                    ...
                    if (!concreteIndices.isEmpty()) {
                        HavenaskAssertions.assertAcked(client().admin().indices().prepareDelete(concreteIndices.toArray(String.class)));
                    }
                }
            }
        }
