...
package org.havenask.test.junit.listeners;
...
public class ReproduceInfoPrinter extends RunListener {
    ...
    protected final Logger logger = LogManager.getLogger(HavenaskTestCase.class);
    ...
    public void testFailure(Failure failure) throws Exception {
        ...
        if (HavenaskClientYamlSuiteTestCase.class.isAssignableFrom(failure.getDescription().getTestClass())) {
    ...
    protected static class GradleMessageBuilder extends ReproduceErrorMessageBuilder {
        ...
        private ReproduceErrorMessageBuilder appendESProperties() {
            appendProperties("tests.havenask.logger.level");
            if (inVerifyPhase()) {
                // these properties only make sense for integration tests
                appendProperties(HavenaskIntegTestCase.TESTS_ENABLE_MOCK_MODULES);
            }
            ...
            appendOpt(HavenaskTestCase.FIPS_SYSPROP, System.getProperty(HavenaskTestCase.FIPS_SYSPROP));
            return this;
        }
