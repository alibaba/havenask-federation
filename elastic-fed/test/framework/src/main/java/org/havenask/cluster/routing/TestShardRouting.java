...
public class TestShardRouting {
    ...
    private static RecoverySource buildRecoveryTarget(boolean primary, ShardRoutingState state) {
        switch (state) {
            ...
            case INITIALIZING:
                if (primary) {
                    return HavenaskTestCase.randomFrom(RecoverySource.EmptyStoreRecoverySource.INSTANCE,
    ...
    private static UnassignedInfo buildUnassignedInfo(ShardRoutingState state) {
        switch (state) {
            case UNASSIGNED:
            case INITIALIZING:
                return new UnassignedInfo(HavenaskTestCase.randomFrom(UnassignedInfo.Reason.values()), "auto generated for test");
            case STARTED:
            case RELOCATING:
                return null;
            default:
