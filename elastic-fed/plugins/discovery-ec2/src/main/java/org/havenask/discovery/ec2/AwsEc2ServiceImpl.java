...
import org.havenask.common.util.LazyInitializable;
...
class AwsEc2ServiceImpl implements AwsEc2Service {
    ...
    private final AtomicReference<LazyInitializable<AmazonEc2Reference, HavenaskException>> lazyClientReference =
            new AtomicReference<>();
    ...
    public AmazonEc2Reference client() {
        final LazyInitializable<AmazonEc2Reference, HavenaskException> clientReference = this.lazyClientReference.get();
    ...
    public void refreshAndClearCache(Ec2ClientSettings clientSettings) {
        final LazyInitializable<AmazonEc2Reference, HavenaskException> newClient = new LazyInitializable<>(
        ...
        final LazyInitializable<AmazonEc2Reference, HavenaskException> oldClient = this.lazyClientReference.getAndSet(newClient);
        if (oldClient != null) {
            oldClient.reset();
        }
    }
    ...
    @Override
    public void close() {
        final LazyInitializable<AmazonEc2Reference, HavenaskException> clientReference = this.lazyClientReference.getAndSet(null);
        if (clientReference != null) {
