    ...
    SynonymMap buildSynonyms(Analyzer analyzer, Reader rules) {
        try {
            ...
            if ("wordnet".equalsIgnoreCase(format)) {
                parser = new HavenaskWordnetSynonymParser(true, expand, lenient, analyzer);
                ((HavenaskWordnetSynonymParser) parser).parse(rules);
            } else {
                parser = new HavenaskSolrSynonymParser(true, expand, lenient, analyzer);
                ((HavenaskSolrSynonymParser) parser).parse(rules);
            }
            return parser.build();
        } catch (Exception e) {
