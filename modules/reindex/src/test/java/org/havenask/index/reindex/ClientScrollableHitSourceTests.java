...
public class ClientScrollableHitSourceTests extends HavenaskTestCase {
    ...
    public void testRetryFail() {
        ...
        assertThat(ex.getCause(), instanceOf(HavenaskRejectedExecutionException.class));
    }
    ...
    private void dotestBasicsWithRetry(int retries, int minFailures, int maxFailures,
        ...
        for (int retry = 0; retry < randomIntBetween(minFailures, maxFailures); ++retry) {
            client.fail(SearchAction.INSTANCE, new HavenaskRejectedExecutionException());
        ...
        for (int i = 0; i < randomIntBetween(1, 10); ++i) {
            ...
            for (int retry = 0; retry < randomIntBetween(minFailures, maxFailures); ++retry) {
                client.fail(SearchScrollAction.INSTANCE, new HavenaskRejectedExecutionException());
                client.awaitOperation();
                ++expectedSearchRetries;
            }
