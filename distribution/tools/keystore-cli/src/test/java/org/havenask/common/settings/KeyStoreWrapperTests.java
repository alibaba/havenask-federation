...
public class KeyStoreWrapperTests extends HavenaskTestCase {
    ...
    public void testFailWhenCannotConsumeSecretStream() throws Exception {
        ...
        try (IndexOutput indexOutput = directory.createOutput("havenask.keystore", IOContext.DEFAULT)) {
            CodecUtil.writeHeader(indexOutput, "havenask.keystore", 3);
    ...
    public void testBackcompatV1() throws Exception {
        ...
        Path configDir = env.configFile();
        SimpleFSDirectory directory = new SimpleFSDirectory(configDir);
        try (IndexOutput output = directory.createOutput("havenask.keystore", IOContext.DEFAULT)) {
            CodecUtil.writeHeader(output, "havenask.keystore", 1);
    ...
    public void testBackcompatV2() throws Exception {
        ...
        try (IndexOutput output = directory.createOutput("havenask.keystore", IOContext.DEFAULT)) {
            ...
            CodecUtil.writeHeader(output, "havenask.keystore", 2);
            output.writeByte((byte) 0); // hasPassword = false
            output.writeString("PKCS12");
    ...
    @AwaitsFix(bugUrl = "https://github.com/alibaba/havenask-federation/issues/468")
    public void testLegacyV3() throws GeneralSecurityException, IOException {
        final Path configDir = createTempDir();
        final Path keystore = configDir.resolve("havenask.keystore");
        try (
            InputStream is = KeyStoreWrapperTests.class.getResourceAsStream("/format-v3-havenask.keystore");
