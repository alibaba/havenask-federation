...
import org.havenask.test.PosixPermissionsResetter;
...
@AwaitsFix(bugUrl = "https://github.com/alibaba/havenask-federation/issues/1")
public class InstallPluginCommandTests extends HavenaskTestCase {
    ...
    static Tuple<Path, Environment> createEnv(FileSystem fs, Function<String, Path> temp) throws IOException {
        ...
        Files.createFile(home.resolve("bin").resolve("havenask"));
        Files.createDirectories(home.resolve("config"));
        Files.createFile(home.resolve("config").resolve("havenask.yml"));
    ...
    public void testInstallFailsIfPreviouslyRemovedPluginFailed() throws Exception {
        ...
        final String expected = String.format(
            Locale.ROOT,
            "found file [%s] from a failed attempt to remove the plugin [failed]; execute [havenask-plugin remove failed]",
            removing
        );
    ...
    public void testBinConflict() throws Exception {
        ...
        Files.createDirectory(binDir);
        Files.createFile(binDir.resolve("somescript"));
        String pluginZip = createPluginUrl("havenask", pluginDir);
        FileAlreadyExistsException e = expectThrows(FileAlreadyExistsException.class, () -> installPlugin(pluginZip, env.v1()));
        assertTrue(e.getMessage(), e.getMessage().contains(env.v2().binFile().resolve("havenask").toString()));
        assertInstallCleaned(env.v2());
    }
    ...
    public void testContainsIntermediateDirectory() throws Exception {
        Tuple<Path, Environment> env = createEnv(fs, temp);
        Path pluginDir = createPluginDir(temp);
        Files.createFile(pluginDir.resolve(PluginInfo.HAVENASK_PLUGIN_PROPERTIES));
        String pluginZip = writeZip(pluginDir, "havenask").toUri().toURL().toString();
        UserException e = expectThrows(UserException.class, () -> installPlugin(pluginZip, env.v1()));
    ...
    public void testOfficialPlugin() throws Exception {
        String url = "https://artifacts.havenask.org/releases/plugins/analysis-icu/"
            + Version.CURRENT
            + "/analysis-icu-"
    ...
    public void testOfficialPluginSnapshot() throws Exception {
        String url = String.format(
            Locale.ROOT,
            "https://artifacts.havenask.org/snapshots/plugins/analysis-icu/%s-abc123/analysis-icu-%s.zip",
            Version.CURRENT,
            Build.CURRENT.getQualifiedVersion()
        );
    ...
    public void testInstallReleaseBuildOfPluginOnSnapshotBuild() {
        ...
        assertThat(e, hasToString(containsString("attempted to install release build of official plugin on snapshot build of Havenask")));
    }

    public void testOfficialPluginStaging() throws Exception {
        String url = "https://artifacts.havenask.org/snapshots/plugins/analysis-icu/"
            + Version.CURRENT
            + "-abc123/analysis-icu-"
            + Build.CURRENT.getQualifiedVersion()
            + ".zip";
    ...
    public void testOfficialPlatformPluginSnapshot() throws Exception {
        String url = String.format(
            Locale.ROOT,
            "https://artifacts.havenask.org/snapshots/plugins/analysis-icu/%s-abc123/analysis-icu-%s-%s.zip",
            Version.CURRENT,
            Platforms.PLATFORM_NAME,
            Build.CURRENT.getQualifiedVersion()
        );
    ...
    public void testPublicKeyIdMismatchToExpectedPublicKeyId() throws Exception {
        ...
        final String url = "https://artifacts.havenask.org/releases/plugins/analysis-icu/"
            + Version.CURRENT
            + "/"
            + icu
            + "-"
    ...
    public void testFailedSignatureVerification() throws Exception {
        ...
        final BiFunction<byte[], PGPSecretKey, String> signature = (b, p) -> {
            ...
            bytes[0] = randomValueOtherThan(b[0], HavenaskTestCase::randomByte);
