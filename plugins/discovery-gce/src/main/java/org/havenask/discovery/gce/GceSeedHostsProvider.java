    ...
    public List<TransportAddress> getSeedAddresses(HostsResolver hostsResolver) {
        ...
        if (this.project == null || this.project.isEmpty() || this.zones == null || this.zones.isEmpty()) {
            throw new IllegalArgumentException("one or more gce discovery settings are missing. " +
                "Check havenask.yml file. Should have [" + GceInstancesService.PROJECT_SETTING.getKey() +
            ...
            for (Instance instance : instances) {
                ...
                try {
                    if (ip_private.equals(ipAddress)) {
                        ...
                        // Test if we have havenask_port metadata defined here
                        if (instance.getMetadata() != null && instance.getMetadata().containsKey("havenask_port")) {
                            Object havenask_port = instance.getMetadata().get("havenask_port");
                            logger.trace("havenask_port is defined with {}", havenask_port);
                            if (havenask_port instanceof String) {
                                address = address.concat(":").concat((String) havenask_port);
                            } else {
                                // Ignoring other values
                                logger.trace("havenask_port is instance of {}. Ignoring...", havenask_port.getClass().getName());
                            }
                        }

                        // ip_private is a single IP Address. We need to build a TransportAddress from it
                        // If user has set `havenask_port` metadata, we don't need to ping all ports
