...
class InternalDistributionDownloadPluginFuncTest extends AbstractGradleFuncTest {
    ...
    def "plugin application fails on non internal build"() {
        ...
        buildFile.text = """
            plugins {
             id 'havenask.internal-distribution-download'
            }
        """
        ...
        then:
        assertOutputContains(result.output, "Plugin 'havenask.internal-distribution-download' is not supported. " +
    ...
    def "resolves current version from local build"() {
        ...
        internalBuild()
        localDistroSetup()
        def distroVersion = VersionProperties.getHavenask()
        buildFile << """
            ...
            tasks.register("setupDistro", Sync) {
                from(havenask_distributions.test_distro.extracted)
                into("build/distro")
            }
        """
    ...
    def "resolves expanded bwc versions from source"() {
        ...
        result.task(":setupDistro").outcome == TaskOutcome.SUCCESS
        assertExtractedDistroIsCreated("distribution/bwc/minor/build/install/havenask-distro",
                'bwc-marker.txt')
    }
    ...
    def "fails on resolving bwc versions with no bundled jdk"() {
        ...
        buildFile << """
            ...
            tasks.register("createExtractedTestDistro") {
                dependsOn havenask_distributions.test_distro.extracted
            }
        """
        when:
    ...
    private void bwcMinorProjectSetup() {
        ...
        new File(bwcSubProjectFolder, 'build.gradle') << """
            ...
            def expandedTask = tasks.register("buildBwcExpandedTask", Copy) {
                from('bwc-marker.txt')
                into('build/install/havenask-distro')
            }
            artifacts {
