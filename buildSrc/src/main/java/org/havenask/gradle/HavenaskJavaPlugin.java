...
import org.havenask.gradle.precommit.PrecommitTaskPlugin;
...
import static org.havenask.gradle.util.Util.toStringable;

/**
 * A wrapper around Gradle's Java plugin that applies our common configuration.
 */
public class HavenaskJavaPlugin implements Plugin<Project> {
    @Override
    public void apply(Project project) {
        ...
        project.getPluginManager().apply(HavenaskTestBasePlugin.class);
        ...
        Consumer<String> disableTransitiveDeps = configName -> {
            ...
            config.getDependencies().all(dep -> {
                if (dep instanceof ModuleDependency
                    ...
                    && dep.getGroup().startsWith("org.havenask") == false) {
    ...
    private static void configureJarManifest(Project project) {
        project.getPlugins().withType(InfoBrokerPlugin.class).whenPluginAdded(manifestPlugin -> {
            ...
            manifestPlugin.add("X-Compile-Havenask-Version", toStringable(VersionProperties::getHavenask));
            manifestPlugin.add("X-Compile-Lucene-Version", toStringable(VersionProperties::getLucene));
            manifestPlugin.add("X-Compile-Havenask-Snapshot", toStringable(() -> Boolean.toString(VersionProperties.isHavenaskSnapshot())));
        });
