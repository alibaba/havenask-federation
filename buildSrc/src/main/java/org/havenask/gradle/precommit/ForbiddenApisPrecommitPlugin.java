...
public class ForbiddenApisPrecommitPlugin extends PrecommitPlugin {
    ...
    public TaskProvider<? extends Task> createTask(Project project) {
        ...
        TaskProvider<ExportHavenaskBuildResourcesTask> resourcesTask = project.getTasks()
            .register("forbiddenApisResources", ExportHavenaskBuildResourcesTask.class);
        ...
        resourcesTask.configure(t -> {
            ...
            t.copy("forbidden/havenask-all-signatures.txt");
            t.copy("forbidden/havenask-test-signatures.txt");
            t.copy("forbidden/http-signatures.txt");
            t.copy("forbidden/havenask-server-signatures.txt");
        });
        project.getTasks().withType(CheckForbiddenApis.class).configureEach(t -> {
            ...
            t.setSignaturesFiles(
                project.files(
                    ...
                    resourcesDir.resolve("forbidden/havenask-all-signatures.txt")
                )
            );
            ...
            if (t.getName().endsWith("Test")) {
                t.setSignaturesFiles(
                    t.getSignaturesFiles()
                        .plus(
                            project.files(
                                resourcesDir.resolve("forbidden/havenask-test-signatures.txt"),
                ...
                t.setSignaturesFiles(
                    t.getSignaturesFiles().plus(project.files(resourcesDir.resolve("forbidden/havenask-server-signatures.txt")))
                );
            }
