...
import org.havenask.index.rankeval.EvalQueryQuality;
...
import org.havenask.index.rankeval.RankEvalSpec;
import org.havenask.index.rankeval.RatedDocument;
import org.havenask.index.rankeval.RatedRequest;
import org.havenask.index.rankeval.RatedSearchHit;
...
import org.havenask.script.mustache.MultiSearchTemplateResponse.Item;
...
import org.havenask.search.aggregations.bucket.terms.Terms.Bucket;
...
import org.havenask.search.profile.query.CollectorResult;
...
import org.havenask.search.suggest.SuggestBuilders;
 ...
 */
public class SearchDocumentationIT extends HavenaskRestHighLevelClientTestCase {
    ...
    public void testSearchRequestAggregations() throws IOException {
        RestHighLevelClient client = highLevelClient();
        {
            ...
            request.add(new IndexRequest("posts").id("1")
                    .source(XContentType.JSON, "company", "Havenask", "age", 20));
            request.add(new IndexRequest("posts").id("2")
                    .source(XContentType.JSON, "company", "Havenask", "age", 30));
            request.add(new IndexRequest("posts").id("3")
                    .source(XContentType.JSON, "company", "Havenask", "age", 40));
        ...
        }
        {
            ...
            SearchResponse searchResponse = client.search(searchRequest, RequestOptions.DEFAULT);
            {
                ...
                Bucket elasticBucket = byCompanyAggregation.getBucketByKey("Havenask"); // <2>
                ...
                try {
                    ...
                    assertThat(message, containsString("org.havenask.search.aggregations.bucket.terms.ParsedStringTerms"));
                    assertThat(message, containsString("org.havenask.search.aggregations.bucket.range.Range"));
                }
            ...
            }
            {
                ...
                for (Aggregation agg : aggregations) {
                    ...
                    if (type.equals(TermsAggregationBuilder.NAME)) {
                        Bucket elasticBucket = ((Terms) agg).getBucketByKey("Havenask");
    ...
    public void testSearchRequestHighlighting() throws IOException {
        RestHighLevelClient client = highLevelClient();
        {
            BulkRequest request = new BulkRequest();
            request.add(new IndexRequest("posts").id("1")
                    .source(XContentType.JSON, "title", "In which order are my Havenask queries executed?", "user",
                            Arrays.asList("foobar", "quxx"), "innerObject", Collections.singletonMap("key", "value")));
            request.add(new IndexRequest("posts").id("2")
                    .source(XContentType.JSON, "title", "Current status and upcoming changes in Havenask", "user",
                            Arrays.asList("foobar", "grault"), "innerObject", Collections.singletonMap("key", "value")));
            request.add(new IndexRequest("posts").id("3")
                    .source(XContentType.JSON, "title", "The Future of Federated Search in Havenask", "user",
        ...
        }
        {
            ...
            searchSourceBuilder.query(QueryBuilders.boolQuery()
                    .should(matchQuery("title", "Havenask"))
            ...
            SearchResponse searchResponse = client.search(searchRequest, RequestOptions.DEFAULT);
            {
                ...
                for (SearchHit hit : hits.getHits()) {
                    ...
                    assertThat(fragments[0].string(), containsString("<em>Havenask</em>"));
    ...
    public void testSearchRequestProfiling() throws IOException {
        RestHighLevelClient client = highLevelClient();
        {
            IndexRequest request = new IndexRequest("posts").id("1")
                    .source(XContentType.JSON, "tags", "havenask", "comments", 123);
        ...
        }
        {
            ...
            searchSourceBuilder.query(QueryBuilders.termQuery("tags", "havenask"));
    ...
    public void testScroll() throws Exception {
        RestHighLevelClient client = highLevelClient();
        {
            BulkRequest request = new BulkRequest();
            request.add(new IndexRequest("posts").id("1")
                    .source(XContentType.JSON, "title", "In which order are my Havenask queries executed?"));
            request.add(new IndexRequest("posts").id("2")
                    .source(XContentType.JSON, "title", "Current status and upcoming changes in Havenask"));
            request.add(new IndexRequest("posts").id("3")
                    .source(XContentType.JSON, "title", "The Future of Federated Search in Havenask"));
            request.setRefreshPolicy(WriteRequest.RefreshPolicy.IMMEDIATE);
            BulkResponse bulkResponse = client.bulk(request, RequestOptions.DEFAULT);
            assertSame(RestStatus.OK, bulkResponse.status());
            assertFalse(bulkResponse.hasFailures());
        }
        {
            ...
            SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
            searchSourceBuilder.query(matchQuery("title", "Havenask"));
    ...
    public void testSearchTemplateWithInlineScript() throws Exception {
        ...
        scriptParams.put("value", "havenask");
        ...
        assertEquals(
            ("{  \"size\" : \"5\",  \"query\": { \"match\" : { \"title\" : \"havenask\" } }}").replaceAll("\\s+", ""),
            source.utf8ToString()
        );
    }
    ...
    public void testMultiSearchTemplateWithInlineScript() throws Exception {
        ...
        String [] searchTerms = {"havenask", "havenask-dashboards"};
    ...
    public void testMultiSearchTemplateWithStoredScript() throws Exception {
        ...
        String [] searchTerms = {"havenask", "dashboards"};
        for (String searchTerm : searchTerms) {
    ...
    private void indexSearchTestData() throws IOException {
        ...
        bulkRequest.add(new IndexRequest("posts").id("1")
                .source(XContentType.JSON, "id", 1, "title", "In which order are my Havenask queries executed?", "user",
                        Arrays.asList("foobar", "quxx"), "innerObject", Collections.singletonMap("key", "value")));
        bulkRequest.add(new IndexRequest("posts").id("2")
                .source(XContentType.JSON, "id", 2, "title", "Current status and upcoming changes in Havenask", "user",
                        Arrays.asList("foobar", "grault"), "innerObject", Collections.singletonMap("key", "value")));
        bulkRequest.add(new IndexRequest("posts").id("3")
                .source(XContentType.JSON, "id", 3, "title", "The Future of Federated Search in Havenask", "user",
                        Arrays.asList("foobar", "quuz"), "innerObject", Collections.singletonMap("key", "value")));
    ...
    private static void indexCountTestData() throws IOException {
        ...
        bulkRequest.add(new IndexRequest("blog").id("2")
            .source(XContentType.JSON, "title", "XYZ Joins Forces with Havenask", "user",
