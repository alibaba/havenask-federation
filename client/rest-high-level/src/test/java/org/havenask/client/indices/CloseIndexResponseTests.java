...
public class CloseIndexResponseTests extends
    AbstractResponseTestCase<org.havenask.action.admin.indices.close.CloseIndexResponse, CloseIndexResponse> {

    @Override
    protected org.havenask.action.admin.indices.close.CloseIndexResponse createServerTestInstance(XContentType xContentType) {
        boolean acknowledged = true;
        final String[] indicesNames = generateRandomStringArray(10, 10, false, true);

        final List<org.havenask.action.admin.indices.close.CloseIndexResponse.IndexResult> indexResults = new ArrayList<>();
        for (String indexName : indicesNames) {
            final Index index = new Index(indexName, randomAlphaOfLength(5));
            if (randomBoolean()) {
                indexResults.add(new org.havenask.action.admin.indices.close.CloseIndexResponse.IndexResult(index));
            } else {
                if (randomBoolean()) {
                    acknowledged = false;
                    Exception exception = randomFrom(new IndexNotFoundException(index), new ActionNotFoundTransportException("test"));
                    indexResults.add(new org.havenask.action.admin.indices.close.CloseIndexResponse.IndexResult(index, exception));
                } else {
                    final int nbShards = randomIntBetween(1, 5);
                    org.havenask.action.admin.indices.close.CloseIndexResponse.ShardResult[] shards =
                        new org.havenask.action.admin.indices.close.CloseIndexResponse.ShardResult[nbShards];
                    for (int i = 0; i < nbShards; i++) {
                        org.havenask.action.admin.indices.close.CloseIndexResponse.ShardResult.Failure[] failures = null;
                        if (randomBoolean()) {
                            acknowledged = false;
                            int nbFailures = randomIntBetween(1, 3);
                            failures = new org.havenask.action.admin.indices.close.CloseIndexResponse.ShardResult.Failure[nbFailures];
                        ...
                        shards[i] = new org.havenask.action.admin.indices.close.CloseIndexResponse.ShardResult(i, failures);
                    }
                    indexResults.add(new org.havenask.action.admin.indices.close.CloseIndexResponse.IndexResult(index, shards));
                }
            }
        }

        final boolean shardsAcknowledged = acknowledged ? randomBoolean() : false;
        return new org.havenask.action.admin.indices.close.CloseIndexResponse(acknowledged, shardsAcknowledged, indexResults);
    }
    ...
    @Override
    protected void assertInstances(final org.havenask.action.admin.indices.close.CloseIndexResponse serverInstance,
        ...
        serverInstance.getIndices().forEach(expectedIndexResult -> {
            ...
            if (expectedIndexResult.getShards() != null) {
                ...
                List<org.havenask.action.admin.indices.close.CloseIndexResponse.ShardResult> failedShardResults =
                    Arrays.stream(expectedIndexResult.getShards())
                        .filter(org.havenask.action.admin.indices.close.CloseIndexResponse.ShardResult::hasFailures)
                        .collect(Collectors.toList());
                ...
                failedShardResults.forEach(failedShardResult -> {
                    ...
                    for (int i = 0; i < failedShardResult.getFailures().length; i++) {
                        org.havenask.action.admin.indices.close.CloseIndexResponse.ShardResult.Failure expectedFailure =
    ...
    private org.havenask.action.admin.indices.close.CloseIndexResponse.ShardResult.Failure newFailure(final String indexName,
                                                                                                        final int shard,
                                                                                                        final String nodeId) {
        Exception exception = randomFrom(new IndexNotFoundException(indexName),
            ...
            new HavenaskStatusException("something", RestStatus.TOO_MANY_REQUESTS));
        return new org.havenask.action.admin.indices.close.CloseIndexResponse.ShardResult.Failure(indexName, shard, exception, nodeId);
    }
}
