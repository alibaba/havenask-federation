...
import org.havenask.action.bulk.BulkProcessor;
...
public class CrudIT extends HavenaskRestHighLevelClientTestCase {
    ...
    public void testDelete() throws IOException {
        ...
        }
        {
            ...
            assertEquals("Havenask exception [type=version_conflict_engine_exception, reason=[" + docId + "]: " +
                "version conflict, required seqNo [2], primary term [2]. current document has seqNo [3] and primary term [1]]",
                exception.getMessage());
            assertEquals("index", exception.getMetadata("havenask.index").get(0));
        }
        {
        ...
        }
        {
            ...
            highLevelClient().index(
                    new IndexRequest("index").id(docId).source(Collections.singletonMap("foo", "bar"))
                .versionType(VersionType.EXTERNAL).version(12), RequestOptions.DEFAULT);
            HavenaskStatusException exception = expectThrows(HavenaskStatusException.class, () -> {
                DeleteRequest deleteRequest = new DeleteRequest("index",  docId).versionType(VersionType.EXTERNAL).version(10);
                execute(deleteRequest, highLevelClient()::delete, highLevelClient()::deleteAsync);
            });
            assertEquals(RestStatus.CONFLICT, exception.status());
            assertEquals("Havenask exception [type=version_conflict_engine_exception, reason=[" +
    ...
    public void testGet() throws IOException {
        {
            ...
            assertEquals("Havenask exception [type=index_not_found_exception, reason=no such index [index]]", exception.getMessage());
            assertEquals("index", exception.getMetadata("havenask.index").get(0));
        }
        IndexRequest index = new IndexRequest("index").id("id");
        ...
        highLevelClient().index(index, RequestOptions.DEFAULT);
        {
            ...
            HavenaskException exception = expectThrows(HavenaskException.class,
                    () -> execute(getRequest, highLevelClient()::get, highLevelClient()::getAsync));
            assertEquals(RestStatus.CONFLICT, exception.status());
            assertEquals("Havenask exception [type=version_conflict_engine_exception, " + "reason=[id]: " +
    ...
    public void testMultiGet() throws IOException {
        {
            ...
            assertEquals("Havenask exception [type=index_not_found_exception, reason=no such index [index]]",
    ...
    public void testGetSource() throws IOException {
        ...
        }
        {
            ...
            HavenaskException exception = expectThrows(HavenaskException.class,
                () -> execute(getRequest, highLevelClient()::getSource, highLevelClient()::getSourceAsync));
            assertEquals(RestStatus.NOT_FOUND, exception.status());
            assertEquals("Havenask exception [type=resource_not_found_exception, " +
        ...
        }
        {
            ...
            HavenaskException exception = expectThrows(HavenaskException.class,
                () -> execute(getRequest, highLevelClient()::getSource, highLevelClient()::getSourceAsync));
            assertEquals("Havenask exception [type=action_request_validation_exception, " +
    ...
    public void testIndex() throws IOException {
        ...
        }
        {
            ...
            assertEquals("Havenask exception [type=illegal_argument_exception, " +
        ...
        }
        {
            ...
            assertEquals(RestStatus.CONFLICT, exception.status());
            assertEquals("Havenask exception [type=version_conflict_engine_exception, reason=[with_create_op_type]: " +
    ...
    public void testUpdate() throws IOException {
        {
            ...
            assertEquals(RestStatus.NOT_FOUND, exception.status());
            assertEquals("Havenask exception [type=document_missing_exception, reason=[_doc][does_not_exist]: document missing]",
                    exception.getMessage());
        }
        {
            IndexRequest indexRequest = new IndexRequest("index").id( "id");
...

            {
                ...
                assertThat(exception.getMessage(), containsString("Havenask exception [type=version_conflict_engine_exception"));
            }
            {
                final UpdateRequest updateRequest = new UpdateRequest("index", "id");
