...
package org.havenask.client.sniff;
...
/**
 * Class responsible for sniffing the http hosts from havenask through the nodes info api and returning them back.
 */
public final class HavenaskNodesSniffer implements NodesSniffer {

    private static final Log logger = LogFactory.getLog(HavenaskNodesSniffer.class);
    ...
    /**
     * Creates a new instance of the Havenask sniffer. It will use the provided {@link RestClient} to fetch the hosts,
     ...
     * @param restClient client used to fetch the hosts from havenask through nodes info api. Usually the same instance
     ...
     */
    public HavenaskNodesSniffer(RestClient restClient) {
        this(restClient, DEFAULT_SNIFF_REQUEST_TIMEOUT, HavenaskNodesSniffer.Scheme.HTTP);
    }
     ...
     *                                  to havenask. Allows to halt the request without any failure, as only the nodes
     *                                  that have responded within this timeout will be returned.
     * @param scheme the scheme to associate sniffed nodes with (as it is not returned by havenask)
     */
    public HavenaskNodesSniffer(RestClient restClient, long sniffRequestTimeoutMillis, Scheme scheme) {
    ...
    /**
     * Calls the havenask nodes info api, parses the response and returns all the found http hosts
     */
    @Override
    ...
    private static Node readNode(String nodeId, JsonParser parser, Scheme scheme) throws IOException {
         ...
         * address on which it is listening. This is useful in Havenask's
