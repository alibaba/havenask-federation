...
import org.havenask.cluster.coordination.PeersResponse;
...
import org.havenask.test.transport.CapturingTransport.CapturedRequest;
import org.havenask.test.transport.StubbableConnectionManager;
import org.havenask.threadpool.ThreadPool.Names;
import org.havenask.transport.ClusterConnectionManager;
import org.havenask.transport.ConnectionManager;
...
import static org.havenask.discovery.PeerFinder.REQUEST_PEERS_ACTION_NAME;
...
public class PeerFinderTests extends HavenaskTestCase {
                    ...
                    for (final Map.Entry<TransportAddress, DiscoveryNode> addressAndNode : reachableNodes.entrySet()) {
                        if (addressAndNode.getKey().equals(transportAddress)) {
                            ...
                            if (discoveryNode.isMasterNode()) {
                                ...
                                listener.onFailure(new HavenaskException("non-master node " + discoveryNode));
                                return;
                            }
                        }
                    }
