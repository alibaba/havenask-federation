...
import org.havenask.common.bytes.AbstractBytesReference;
...
public class IndicesRequestCacheTests extends HavenaskTestCase {
    ...
    public void testBasicOperationsCache() throws Exception {
        ...
        DirectoryReader reader = HavenaskDirectoryReader.wrap(DirectoryReader.open(writer),
    ...
    public void testCacheDifferentReaders() throws Exception {
        ...
        writer.addDocument(newDoc(0, "foo"));
        DirectoryReader reader = HavenaskDirectoryReader.wrap(DirectoryReader.open(writer), new ShardId("foo", "bar", 1));
        TermQueryBuilder termQuery = new TermQueryBuilder("id", "0");
        BytesReference termBytes = XContentHelper.toXContent(termQuery, XContentType.JSON, false);
        if (randomBoolean()) {
            writer.flush();
        ...
        DirectoryReader secondReader = HavenaskDirectoryReader.wrap(DirectoryReader.open(writer), new ShardId("foo", "bar", 1));
    ...
    public void testEviction() throws Exception {
        final ByteSizeValue size;
        {
            ...
            writer.updateDocument(new Term("id", "0"), newDoc(0, "bar"));
            DirectoryReader secondReader = HavenaskDirectoryReader.wrap(DirectoryReader.open(writer),
                new ShardId("foo", "bar", 1));
            TestEntity secondEntity = new TestEntity(requestCacheStats, indexShard);
        ...
        DirectoryReader thirdReader = HavenaskDirectoryReader.wrap(DirectoryReader.open(writer),
            new ShardId("foo", "bar", 1));
