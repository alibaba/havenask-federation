...
import static org.havenask.search.query.QueryPhase.pointsHaveDuplicateData;
import static org.havenask.search.query.TopDocsCollectorContext.hasInfMaxScore;
...
public class QueryPhaseTests extends IndexShardTestCase {
    ...
    public void testMaxScoreQueryVisitor() {
        ...
        Query query = new HavenaskToParentBlockJoinQuery(new MatchAllDocsQuery(), producer, ScoreMode.Avg, "nested");
        assertTrue(hasInfMaxScore(query));

        query = new HavenaskToParentBlockJoinQuery(new MatchAllDocsQuery(), producer, ScoreMode.None, "nested");
        assertFalse(hasInfMaxScore(query));


        for (Occur occur : Occur.values()) {
            query = new BooleanQuery.Builder()
                .add(new HavenaskToParentBlockJoinQuery(new MatchAllDocsQuery(), producer, ScoreMode.Avg, "nested"), occur)
                .build();
            if (occur == Occur.MUST) {
                assertTrue(hasInfMaxScore(query));
            } else {
