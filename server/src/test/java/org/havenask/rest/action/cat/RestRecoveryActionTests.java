...
public class RestRecoveryActionTests extends HavenaskTestCase {
    ...
    public void testRestRecoveryAction() {
        ...
        for (int i = 0; i < successfulShards; i++) {
            ...
            final List<Object> expectedValues = Arrays.asList(
                    "index",
                    i,
                    XContentHavenaskExtension.DEFAULT_DATE_PRINTER.print(state.getTimer().startTime()),
                    state.getTimer().startTime(),
                    XContentHavenaskExtension.DEFAULT_DATE_PRINTER.print(state.getTimer().stopTime()),
