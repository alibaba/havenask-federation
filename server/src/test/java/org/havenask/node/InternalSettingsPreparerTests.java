...
import org.havenask.common.settings.SecureSetting;
...
public class InternalSettingsPreparerTests extends HavenaskTestCase {
    ...
    public void testDefaultClusterName() {
        Settings settings = InternalSettingsPreparer.prepareSettings(Settings.EMPTY);
        assertEquals("havenask", settings.get("cluster.name"));
    ...
    public void testGarbageIsNotSwallowed() throws IOException {
        try {
            ...
            Files.copy(garbage, config.resolve("havenask.yml"));
            ...
            assertEquals("Failed to load settings from [havenask.yml]", e.getMessage());
        }
    }

    @AwaitsFix(bugUrl = "https://github.com/alibaba/havenask-federation/issues/358")
    public void testYamlNotAllowed() throws IOException {
        InputStream yaml = getClass().getResourceAsStream("/config/havenask.yml");
        Path config = homeDir.resolve("config");
        Files.createDirectory(config);
        Files.copy(yaml, config.resolve("havenask.yaml"));
        ...
        assertEquals("havenask.yaml was deprecated in 5.5.0 and must be renamed to havenask.yml", e.getMessage());
    }

    @AwaitsFix(bugUrl = "https://github.com/alibaba/havenask-federation/issues/358")
    public void testJsonNotAllowed() throws IOException {
        InputStream yaml = getClass().getResourceAsStream("/config/havenask.json");
        Path config = homeDir.resolve("config");
        Files.createDirectory(config);
        Files.copy(yaml, config.resolve("havenask.json"));
        SettingsException e = expectThrows(SettingsException.class, () -> InternalSettingsPreparer.prepareEnvironment(
                Settings.builder().put(baseEnvSettings).build(), emptyMap(), null, DEFAULT_NODE_NAME_SHOULDNT_BE_CALLED));
        assertEquals("havenask.json was deprecated in 5.5.0 and must be converted to havenask.yml", e.getMessage());
    }
