...
public class IndexShardOperationPermitsTests extends HavenaskTestCase {
    ...
    public static void setupThreadPool() {
        ...
        threadPool = new TestThreadPool("IndexShardOperationPermitsTests",
            Settings.builder()
                ...
                .build());
        assertThat(threadPool.executor(ThreadPool.Names.WRITE), instanceOf(HavenaskThreadPoolExecutor.class));
        assertThat(((HavenaskThreadPoolExecutor) threadPool.executor(ThreadPool.Names.WRITE)).getCorePoolSize(),
            equalTo(writeThreadPoolSize));
        assertThat(((HavenaskThreadPoolExecutor) threadPool.executor(ThreadPool.Names.WRITE)).getMaximumPoolSize(),
            equalTo(writeThreadPoolSize));
        assertThat(((HavenaskThreadPoolExecutor) threadPool.executor(ThreadPool.Names.WRITE)).getQueue().remainingCapacity(),
            ...
            try {
                ...
                if (closeAfterBlocking) {
                    assertThat(e.getCause(), either(instanceOf(DummyException.class))
                        .or(instanceOf(HavenaskRejectedExecutionException.class))
