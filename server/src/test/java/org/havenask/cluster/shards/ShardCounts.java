...
public class ShardCounts {
    ...
    public static ShardCounts forDataNodeCount(int dataNodes) {
        ...
        int mainIndexReplicas = HavenaskTestCase.between(0, dataNodes - 1);
        int mainIndexShards = HavenaskTestCase.between(1, 10);
        ...
        int shardsPerNode = (int) Math.ceil((double) totalShardsInIndex / dataNodes) + HavenaskTestCase.between(0, 10);
        int totalCap = shardsPerNode * dataNodes;

        int failingIndexShards;
        int failingIndexReplicas;
        if (dataNodes > 1 && HavenaskTestCase.frequently()) {
            failingIndexShards = Math.max(1, totalCap - totalShardsInIndex);
            failingIndexReplicas = HavenaskTestCase.between(1, dataNodes - 1);
        } else {
            failingIndexShards = totalCap - totalShardsInIndex + HavenaskTestCase.between(1, 10);
