...
public class BatchedRerouteServiceTests extends HavenaskTestCase {
            ...
            batchedRerouteService.reroute("iteration " + i,
                randomFrom(EnumSet.allOf(Priority.class)), ActionListener.wrap(
                    r -> {
                        ...
                        if (rarely()) {
                            throw new HavenaskException("failure during notification");
                        }
                    }, e -> {
                        countDownLatch.countDown();
                        if (randomBoolean()) {
                            throw new HavenaskException("failure during failure notification", e);
                        }
                    }));
            if (rarely()) {
