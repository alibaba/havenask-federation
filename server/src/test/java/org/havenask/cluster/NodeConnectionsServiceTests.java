...
import static org.havenask.cluster.NodeConnectionsService.CLUSTER_NODE_RECONNECT_INTERVAL_SETTING;
...
public class NodeConnectionsServiceTests extends HavenaskTestCase {
    ...
    public void testOnlyBlocksOnConnectionsToNewNodes() throws Exception {
        ...
        try {
            ...
            expectThrows(HavenaskTimeoutException.class, () -> future3.actionGet(timeValueMillis(scaledRandomIntBetween(1, 1000))));
            ...
            expectThrows(HavenaskTimeoutException.class, () -> future6.actionGet(timeValueMillis(scaledRandomIntBetween(1, 1000))));
    ...
    @TestLogging(reason="testing that DEBUG-level logging is reasonable", value="org.havenask.cluster.NodeConnectionsService:DEBUG")
    public void testDebugLogging() throws IllegalAccessException {
        ...
        try {
            appender.start();
            Loggers.addAppender(LogManager.getLogger("org.havenask.cluster.NodeConnectionsService"), appender);
            for (DiscoveryNode targetNode : targetNodes) {
                if (disconnectedNodes.contains(targetNode)) {
                    appender.addExpectation(new MockLogAppender.SeenEventExpectation("connecting to " + targetNode,
                        "org.havenask.cluster.NodeConnectionsService", Level.DEBUG,
            ...
            Loggers.removeAppender(LogManager.getLogger("org.havenask.cluster.NodeConnectionsService"), appender);
            appender.stop();
        }        for (DiscoveryNode disconnectedNode : disconnectedNodes) {
            transportService.disconnectFromNode(disconnectedNode);
        }
