...
import org.havenask.action.support.replication.ReplicationOperation.ReplicaResponse;
...
import org.havenask.transport.TestTransportChannel;
...
public class TransportReplicationActionTests extends HavenaskTestCase {
    ...
    */
    ...
    public void testNoRerouteOnStaleClusterState() {
        ...
        AllocationService allocationService = HavenaskAllocationTestCase.createAllocationService();
        ClusterState updatedState = HavenaskAllocationTestCase.startShardsAndReroute(allocationService, state, relocationTarget);
    ...
    public void testStalePrimaryShardOnReroute() {
        ...
        if (timeout) {
            ...
            assertPhase(task, "waiting_on_primary");
            transport.handleRemoteError(capturedRequests[0].requestId, randomRetryPrimaryException(shardId));
            assertListenerThrows("must throw index not found exception", listener, HavenaskException.class);
            assertPhase(task, "failed");
        } else {
            assertThat(listener.isDone(), equalTo(false));
    ...
    public void testPrimaryReference() {
        ...
        shard.runUnderPrimaryPermit(() ->
            primary.perform(request, ActionTestUtils.assertNoFailureListener(r -> {
                final HavenaskException exception = new HavenaskException("testing");
    ...
    public void testReplicaProxy() throws InterruptedException, ExecutionException {
        ...
        if (randomBoolean()) {
            ...
            transport.handleRemoteError(captures[0].requestId, new HavenaskException("simulated"));
            assertTrue(listener.isDone());
            assertListenerThrows("listener should reflect remote error", listener, HavenaskException.class);
        } else {
        ...
        proxy.failShardIfNeeded(replica, primaryTerm, "test", new HavenaskException("simulated"),
    ...
    public void testCounterOnPrimary() throws Exception {
        ...
        new TestAction(Settings.EMPTY, "internal:testAction2", transportService, clusterService, shardStateAction, threadPool) {
            ...
            protected void shardOperationOnPrimary(Request shardRequest, IndexShard primary,
                ...
                if (throwExceptionOnRun) {
                    throw new HavenaskException("simulated exception, during shardOperationOnPrimary");
                } else if (respondWithError) {
                    listener.onFailure(new HavenaskException("simulated exception, as a response"));
                } else {
        ...
        try {
            ...
            if (throwExceptionOnRun || respondWithError) {
                ...
                assertThat(cause, instanceOf(HavenaskException.class));
        ...
        TestAction action = new TestAction(Settings.EMPTY, "internal:testActionWithExceptions", transportService, clusterService,
            ...
            protected void shardOperationOnReplica(Request shardRequest, IndexShard replica, ActionListener<ReplicaResult> listener) {
                ActionListener.completeWith(listener, () -> {
                    ...
                    if (throwException) {
                        throw new HavenaskException("simulated");
                    }
    ...
    /**
     * test throwing a {@link org.havenask.action.support.replication.TransportReplicationAction.RetryOnReplicaException}
     * causes a retry
     */
