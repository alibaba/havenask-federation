...
import org.havenask.common.LocalTimeOffset.Gap;
import org.havenask.common.LocalTimeOffset.Overlap;
...
public class LocalTimeOffsetTests extends HavenaskTestCase {
    ...
    private void assertFixedOffset(ZoneId zone, long offsetMillis) {
        ...
        long min = randomLong();
        long max = randomValueOtherThan(min, HavenaskTestCase::randomLong);
        if (min > max) {
            long s = min;
            min = max;
            max = s;
