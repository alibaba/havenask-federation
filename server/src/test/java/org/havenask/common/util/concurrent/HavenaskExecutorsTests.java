...
/**
 * Tests for HavenaskExecutors and its components like HavenaskAbortPolicy.
 */
public class HavenaskExecutorsTests extends HavenaskTestCase {
    ...
    public void testFixedForcedExecution() throws Exception {
        HavenaskThreadPoolExecutor executor =
                HavenaskExecutors.newFixed(getName(), 1, 1, HavenaskExecutors.daemonThreadFactory("test"), threadContext);
    ...
    public void testScaleUp() throws Exception {
        ...
        ThreadPoolExecutor pool =
                HavenaskExecutors.newScaling(getClass().getName() + "/" + getTestName(), min, max, between(1, 100), randomTimeUnit(),
                    HavenaskExecutors.daemonThreadFactory("test"), threadContext);
    ...
    public void testScaleDown() throws Exception {
        ...
        final ThreadPoolExecutor pool =
                HavenaskExecutors.newScaling(getClass().getName() + "/" + getTestName(), min, max, between(1, 100), TimeUnit.MILLISECONDS,
                    HavenaskExecutors.daemonThreadFactory("test"), threadContext);
        assertThat("Min property", pool.getCorePoolSize(), equalTo(min));
        assertThat("Max property", pool.getMaximumPoolSize(), equalTo(max));
    ...
    public void testRejectionMessageAndShuttingDownFlag() throws InterruptedException {
        ...
        final CountDownLatch latch = new CountDownLatch(1);
        HavenaskThreadPoolExecutor executor =
                HavenaskExecutors.newFixed(getName(), pool, queue, HavenaskExecutors.daemonThreadFactory("dummy"), threadContext);
        try {
            ...
            try {
                ...
                assertThat(message, containsString("on HavenaskThreadPoolExecutor[name = " + getName()));
    ...
    public void testNodeProcessorsBound() {
        runProcessorsBoundTest(HavenaskExecutors.NODE_PROCESSORS_SETTING);
    }

    public void testProcessorsBound() {
        runProcessorsBoundTest(HavenaskExecutors.PROCESSORS_SETTING);
    }
