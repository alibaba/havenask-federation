...
/**
 * Tests for the automatic queue resizing of the {@code QueueResizingHavenaskThreadPoolExecutorTests}
 * based on the time taken for each event.
 */
public class QueueResizingHavenaskThreadPoolExecutorTests extends HavenaskTestCase {
    ...
    public void testExactWindowSizeAdjustment() throws Exception {
        ...
        QueueResizingHavenaskThreadPoolExecutor executor =
                new QueueResizingHavenaskThreadPoolExecutor(
                        "test-threadpool", threads, threads, 1000,
                        TimeUnit.MILLISECONDS, queue, 10, 1000, fastWrapper(),
                        measureWindow, TimeValue.timeValueMillis(1), HavenaskExecutors.daemonThreadFactory("queuetest"),
                        new HavenaskAbortPolicy(), context);
    ...
    public void testAutoQueueSizingDown() throws Exception {
        ...
        QueueResizingHavenaskThreadPoolExecutor executor =
                new QueueResizingHavenaskThreadPoolExecutor(
                        ...
                        HavenaskExecutors.daemonThreadFactory("queuetest"), new HavenaskAbortPolicy(), context);
        executor.prestartAllCoreThreads();
        logger.info("--> executor: {}", executor);
    ...
    private void executeTask(QueueResizingHavenaskThreadPoolExecutor executor, int times) {
