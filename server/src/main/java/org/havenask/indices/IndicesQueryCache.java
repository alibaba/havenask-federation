...
import org.havenask.common.lucene.ShardCoreKeyMap;
...
public class IndicesQueryCache implements QueryCache, Closeable {
    ...
    public IndicesQueryCache(Settings settings) {
        ...
        if (INDICES_QUERIES_CACHE_ALL_SEGMENTS_SETTING.get(settings)) {
            cache = new HavenaskLRUQueryCache(count, size.getBytes(), context -> true, 1f);
        } else {
            cache = new HavenaskLRUQueryCache(count, size.getBytes());
        }
    ...
    private class HavenaskLRUQueryCache extends LRUQueryCache {

        HavenaskLRUQueryCache(int maxSize, long maxRamBytesUsed, Predicate<LeafReaderContext> leavesToCache, float skipFactor) {
            super(maxSize, maxRamBytesUsed, leavesToCache, skipFactor);
        }

        HavenaskLRUQueryCache(int maxSize, long maxRamBytesUsed) {
