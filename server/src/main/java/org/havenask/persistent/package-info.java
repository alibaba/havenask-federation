 ...
 * by the {@link org.havenask.persistent.PersistentTasksService}, which is using
 * {@link org.havenask.persistent.PersistentTasksClusterService} to update cluster state with the record about running persistent
 * task.
 * <p>
 * 2. The master node updates the {@link org.havenask.persistent.PersistentTasksCustomMetadata} in the cluster state to indicate
 * that there is a new persistent task running in the system.
 * <p>
 * 3. The {@link org.havenask.persistent.PersistentTasksNodeService} running on every node in the cluster monitors changes in
 * the cluster state and starts execution of all new tasks assigned to the node it is running on.
 * <p>
 * 4. If the task fails to start on the node, the {@link org.havenask.persistent.PersistentTasksNodeService} uses the
 * {@link org.havenask.persistent.PersistentTasksCustomMetadata} to notify the
 * {@link org.havenask.persistent.PersistentTasksService}, which reassigns the action to another node in the cluster.
 * <p>
 ...
 * <p>
 * 6. The {@link org.havenask.persistent.RemovePersistentTaskAction} action can be also used to remove the persistent task.
 */
