...
import org.havenask.common.util.concurrent.HavenaskAbortPolicy;
...
public interface Scheduler {
    ...
    static ScheduledThreadPoolExecutor initScheduler(Settings settings) {
        final ScheduledThreadPoolExecutor scheduler = new SafeScheduledThreadPoolExecutor(1,
                HavenaskExecutors.daemonThreadFactory(settings, "scheduler"), new HavenaskAbortPolicy());
     ...
     * @throws HavenaskRejectedExecutionException if the task cannot be scheduled for execution
     */
    ...
    final class ReschedulingRunnable extends AbstractRunnable implements Cancellable {
        ...
        public void onAfter() {
            ...
            if (run) {
                try {
                    scheduler.schedule(this, interval, executor);
                } catch (final HavenaskRejectedExecutionException e) {
                    onRejection(e);
                }
            }
        }
    ...
    class SafeScheduledThreadPoolExecutor extends ScheduledThreadPoolExecutor {
        ...
        protected void afterExecute(Runnable r, Throwable t) {
            ...
            if (r instanceof RunnableFuture && ((RunnableFuture<?>) r).isDone()) {
                ...
                // RunnableFuture.get()
                ExceptionsHelper.reThrowIfNotNull(HavenaskExecutors.rethrowErrors(r));
            }
        }
    }
}
