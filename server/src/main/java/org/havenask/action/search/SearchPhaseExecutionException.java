...
public class SearchPhaseExecutionException extends HavenaskException {
    ...
    public RestStatus status() {
        if (shardFailures.length == 0) {
            // if no successful shards, the failure can be due to HavenaskRejectedExecutionException during fetch phase
    ...
    public Throwable getCause() {
        ...
        if (cause == null) {
            ...
            for (HavenaskException rootCause : guessRootCauses()) {
                return rootCause;
            }
        }
        return cause;
    ...
    public HavenaskException[] guessRootCauses() {
        ShardOperationFailedException[] failures = ExceptionsHelper.groupBy(shardFailures);
        List<HavenaskException> rootCauses = new ArrayList<>(failures.length);
        for (ShardOperationFailedException failure : failures) {
            HavenaskException[] guessRootCauses = HavenaskException.guessRootCauses(failure.getCause());
            rootCauses.addAll(Arrays.asList(guessRootCauses));
        }
        return rootCauses.toArray(new HavenaskException[0]);
    }
