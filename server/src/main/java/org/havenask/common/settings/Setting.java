...
import org.havenask.common.unit.MemorySizeValue;
 ...
 * All settings inside havenask or in any of the plugins should use this type-safe and generic settings infrastructure
...
public class Setting<T> implements ToXContentObject {
    ...
    private T get(Settings settings, boolean validate) {
        ...
        try {
            ...
            return parsed;
        } catch (HavenaskParseException ex) {
    ...
    String innerGetRaw(final Settings settings) {
        ...
        if (secureSettings != null && secureSettings.getSettingNames().contains(getKey())) {
            throw new IllegalArgumentException("Setting [" + getKey() + "] is a non-secure setting" +
                " and must be stored inside havenask.yml, but was found inside the Havenask keystore");
        }
    ...
    void checkDeprecation(Settings settings) {
        ...
        if (this.isDeprecated() && this.exists(settings)) {
            ...
            Settings.DeprecationLoggerHolder.deprecationLogger
                .deprecate(key, "[{}] setting was deprecated in Havenask and will be removed in a future release! "
    ...
    private static String arrayToParsableString(List<String> array) {
        try {
            ...
            throw new HavenaskException(ex);
        }
    }
