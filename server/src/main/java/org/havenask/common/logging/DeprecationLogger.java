 ...
 * for deprecation logger. For instance <code>DeprecationLogger.getLogger("org.havenask.test.SomeClass")</code> will
 * result in a deprecation logger with name <code>org.havenask.deprecation.test.SomeClass</code>. This allows to use a
...
public class DeprecationLogger {
     ...
     * prefixes the logger name with "deprecation", if it starts with "org.havenask.",
     * it replaces "org.havenask" with "org.havenask.deprecation" to maintain
     * the "org.havenask" namespace.
     */
    ...
    private static Logger getDeprecatedLoggerForName(String name) {
        if (name.startsWith("org.havenask")) {
            name = name.replace("org.havenask.", "org.havenask.deprecation.");
        } else {
    ...
    public class DeprecationLoggerBuilder {
        ...
        public DeprecationLoggerBuilder withDeprecation(String key, String msg, Object[] params) {
            HavenaskLogMessage deprecationMessage = new DeprecatedMessage(key, HeaderWarning.getXOpaqueId(), msg, params);
