...
import org.havenask.common.geo.parsers.CoordinateNode;
    ...
    POINT("point") {
        ...
        CoordinateNode validate(CoordinateNode coordinates, boolean coerce) {
            if (coordinates.isEmpty()) {
                ...
                throw new HavenaskParseException("multipoint data provided when single point data expected.");
            }
            return coordinates;
        }
    },
        ...
        CoordinateNode validate(CoordinateNode coordinates, boolean coerce) {
            if (coordinates.children == null || coordinates.children.isEmpty()) {
                if (coordinates.coordinate != null) {
                    throw new HavenaskParseException("single coordinate found when expecting an array of " +
                        "coordinates. change type to point or change data to an array of >0 coordinates");
                }
                throw new HavenaskParseException("no data provided for multipoint object when expecting " +
    ...
    LINESTRING("linestring") {
        ...
        CoordinateNode validate(CoordinateNode coordinates, boolean coerce) {
            if (coordinates.children.size() < 2) {
                throw new HavenaskParseException("invalid number of points in LineString (found [{}] - must be >= 2)",
    ...
    MULTILINESTRING("multilinestring") {
        ...
        CoordinateNode validate(CoordinateNode coordinates, boolean coerce) {
            if (coordinates.children.size() < 1) {
                throw new HavenaskParseException("invalid number of lines in MultiLineString (found [{}] - must be >= 1)",
                    coordinates.children.size());
            }
            return coordinates;
        }
    ...
    POLYGON("polygon") {
        ...
        void validateLinearRing(CoordinateNode coordinates, boolean coerce) {
            if (coordinates.children == null || coordinates.children.isEmpty()) {
                ...
                throw new HavenaskParseException(error);
            }

            int numValidPts = coerce ? 3 : 4;
            if (coordinates.children.size() < numValidPts) {
                throw new HavenaskParseException("invalid number of points in LinearRing (found [{}] - must be >= [{}])",
