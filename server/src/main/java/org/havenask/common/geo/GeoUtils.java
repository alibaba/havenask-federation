...
import org.havenask.common.xcontent.XContentSubParser;
...
import org.havenask.index.fielddata.GeoPointValues;
...
public class GeoUtils {
     ...
     */
    public static GeoPoint parseGeoPoint(XContentParser parser) throws IOException, HavenaskParseException {
        return parseGeoPoint(parser, new GeoPoint());
    }


    public static GeoPoint parseGeoPoint(XContentParser parser, GeoPoint point) throws IOException, HavenaskParseException {
     ...
     */
    public static GeoPoint parseGeoPoint(Object value, final boolean ignoreZValue) throws HavenaskParseException {
     ...
     * Array: two or more elements, the first element is longitude, the second is latitude, the rest is ignored if ignoreZValue is true
     */
    public static GeoPoint parseGeoPoint(Object value, GeoPoint point, final boolean ignoreZValue) throws HavenaskParseException {
        try (XContentParser parser = new MapXContentParser(NamedXContentRegistry.EMPTY, LoggingDeprecationHandler.INSTANCE,
            ...
            throw new HavenaskParseException("error parsing geopoint", ex);
        }
    }
    ...
    public static GeoPoint parseGeoPoint(XContentParser parser, GeoPoint point, final boolean ignoreZValue, EffectivePoint effectivePoint)
        ...
        if(parser.currentToken() == Token.START_OBJECT) {
                    ...
                    if (subParser.currentToken() == Token.FIELD_NAME) {
                        ...
                        if (LATITUDE.equals(field)) {
                            ...
                            if (subParser.nextToken() == Token.VALUE_STRING) {
                                ...
                                throw new HavenaskParseException("geohash must be a string");
                            }
                        } else {
                            throw new HavenaskParseException("field must be either [{}], [{}] or [{}]", LATITUDE, LONGITUDE, GEOHASH);
                        }
                    } else {
                        throw new HavenaskParseException("token [{}] not allowed", subParser.currentToken());
                    }
                }
            }
            if (geohash != null) {
                if(!Double.isNaN(lat) || !Double.isNaN(lon)) {
                    throw new HavenaskParseException("field must be either lat/lon or geohash");
                } else {
                    return point.parseGeoHash(geohash, effectivePoint);
                }
            } else if (numberFormatException != null) {
                throw new HavenaskParseException("[{}] and [{}] must be valid double values", numberFormatException, LATITUDE,
                    LONGITUDE);
            } else if (Double.isNaN(lat)) {
                throw new HavenaskParseException("field [{}] missing", LATITUDE);
            } else if (Double.isNaN(lon)) {
                throw new HavenaskParseException("field [{}] missing", LONGITUDE);
            } else {
                ...
                while (subParser.nextToken() != Token.END_ARRAY) {
                    if (subParser.currentToken() == Token.VALUE_NUMBER) {
                        ...
                        if (element == 1) {
                            ...
                            throw new HavenaskParseException("[geo_point] field type does not accept > 3 dimensions");
                        }
                    } else {
                        throw new HavenaskParseException("numeric value expected");
                    }
                }
            }
            return point.reset(lat, lon);
            ...
            throw new HavenaskParseException("geo_point expected");
        }
    }
     ...
     */
    public static int parsePrecision(XContentParser parser) throws IOException, HavenaskParseException {
    ...
    /**
     * Checks that the precision is within range supported by havenask - between 1 and 12
     *
