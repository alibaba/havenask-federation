...
abstract class GeoJsonParser {
    protected static ShapeBuilder parse(XContentParser parser, AbstractShapeGeometryFieldMapper shapeMapper)
                ...
                if (token == XContentParser.Token.FIELD_NAME) {
                    ...
                    if (ShapeParser.FIELD_TYPE.match(fieldName, subParser.getDeprecationHandler())) {
                        ...
                        if (coordinateNode != null && tempNode.numDimensions() != coordinateNode.numDimensions()) {
                            throw new HavenaskParseException("Exception parsing coordinates: " +
        ...
        if (malformedException != null) {
            throw new HavenaskParseException(malformedException);
        } else if (shapeType == null) {
            throw new HavenaskParseException("shape type not included");
            ...
            throw new HavenaskParseException("field [{}] is supported for [{}] only", CircleBuilder.FIELD_RADIUS,
                CircleBuilder.TYPE);
        }
    ...
    private static CoordinateNode parseCoordinates(XContentParser parser, boolean ignoreZValue) throws IOException {
        if (parser.currentToken() == XContentParser.Token.START_OBJECT) {
            parser.skipChildren();
            parser.nextToken();
            throw new HavenaskParseException("coordinates cannot be specified as objects");
        }
    ...
    static GeometryCollectionBuilder parseGeometries(XContentParser parser, AbstractShapeGeometryFieldMapper mapper) throws
        ...
        if (parser.currentToken() != XContentParser.Token.START_ARRAY) {
            throw new HavenaskParseException("geometries must be an array of geojson objects");
        }
