...
import org.havenask.index.mapper.GeoPointFieldMapper.GeoPointFieldType;
...
public abstract class DecayFunctionBuilder<DFB extends DecayFunctionBuilder<DFB>> extends ScoreFunctionBuilder<DFB> {
    ...
    private AbstractDistanceScoreFunction parseNumberVariable(XContentParser parser, QueryShardContext context,
        ...
        while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                throw new HavenaskParseException("parameter [{}] not supported!", parameterName);
            }
        }
        if (!scaleFound || !refFound) {
            throw new HavenaskParseException("both [{}] and [{}] must be set for numeric fields.", DecayFunctionBuilder.SCALE,
    ...
    private AbstractDistanceScoreFunction parseGeoVariable(XContentParser parser, QueryShardContext context,
        ...
        if (origin == null || scaleString == null) {
            throw new HavenaskParseException("[{}] and [{}] must be set for geo fields.", DecayFunctionBuilder.ORIGIN,
    ...
    private AbstractDistanceScoreFunction parseDateVariable(XContentParser parser, QueryShardContext context,
        ...
        if (scaleString == null) {
            throw new HavenaskParseException("[{}] must be set for date fields.", DecayFunctionBuilder.SCALE);
        }
